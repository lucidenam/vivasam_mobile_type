...
class Plyr extends Component {
  constructor() {
    ...
    this.doOnOrientationChange = debounce(this.doOnOrientationChange, 100);
  }
  ...
  componentDidMount() {
    window.addEventListener('resize', this.doOnOrientationChange, false);
    ...
    if (this.player) {
      ...
      this.player.on('enterfullscreen', () => {
          document.getElementById('viewer').classList.add('full');
        this.props.onEnterFullscreen && this.props.onEnterFullscreen();
      });

      this.player.on('exitfullscreen', () => {
          document.getElementById('viewer').classList.remove('full');
      ...
      this.player.on('click', (e) => {
          var _this = this;
          if(e.target.dataset.plyr !== 'play'){
              _this.player.elements.container.classList.toggle('plyr--hide-controls');
          }
      });
    }
  }
  ...
  doOnOrientationChange = () => {
    var target = document.getElementById("viewer_image");
    if(target && target.style){
        switch(window.orientation) {
            case 90:
            target.style.height = (window.innerHeight - 16 - 55) + 'px';
            break;
            case -90:
            target.style.height = (window.innerHeight - 16 - 55) + 'px';
            break;
