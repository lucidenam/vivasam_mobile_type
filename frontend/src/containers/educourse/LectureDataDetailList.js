...
import {getContentInfo, getEducourseUnitDataList, getUnitSubTypeList} from 'lib/api';
...
class LectureDataDetailList extends Component {
    state = {
        page: {
            number: 0,
            size: 20,
    ...
    getEducourseUnitDataList = async (textbookCd, classCd, type1Cd, type2Cd, isMore) => {
        try {
            ...
            const response = await getEducourseUnitDataList(classCd, type1Cd, type2Cd, isMore ? number + 1 : 0 , size);
            if(this._isMounted) {
                this.setState({
    ...
    getUnitSubTypeList = async (classCd, type1Cd) => {
        const response = await getUnitSubTypeList(classCd, type1Cd);
        if(this._isMounted) {
            this.setState({
                unitDetailTypes: response.data
            });
        }
    ...
    handleShowMore = () => {
        const {textbookCd, classCd, type1Cd} = this.props;
        const {type2Cd} = this.state;
        this.getEducourseUnitDataList(textbookCd, classCd, type1Cd, type2Cd, true);
    }
    ...
    shouldComponentUpdate(nextProps, nextState) {
        const {textbookCd, classCd, type1Cd, type2Cd} = nextProps;
        if(this.props.textbook !== nextProps.textbook
            || this.props.classCd !== nextProps.classCd
            || this.props.type1Cd !== nextProps.type1Cd
            || this.props.type2Cd !== nextProps.type2Cd) {
            this.getUnitSubTypeList(classCd, type1Cd);
            this.getEducourseUnitDataList(textbookCd, classCd, type1Cd, type2Cd);
            if(this._isMounted) {
                this.setState({
                    list: null,
                })
        ...
        if(this.state.type2Cd !== nextState.type2Cd) {
            this.getEducourseUnitDataList(textbookCd, classCd, type1Cd, nextState.type2Cd);
            if(this._isMounted) {
                this.setState({
                    list: null,
                })
    ...
    componentDidMount() {
        ...
        const {textbookCd, classCd, type1Cd, type2Cd} = this.props;
        this.getUnitSubTypeList(classCd, type1Cd);
        this.getEducourseUnitDataList(textbookCd, classCd, type1Cd, type2Cd);
    }
    ...
    render() {
        const {textbookCd, classCd, type1Cd } = this.props;
        const { list, page, type1Nm, type2Cd, unitDetailTypes, isLoading } = this.state;
        if(list === null ) {
        ...
        //멀티미디어자료, 이미지자료
        if(type1Cd === '1110003' || type1Cd === '1110005') {
            ...
            container = <ThumbnailList
                            metaData={metaData}
                            type={type1Cd === '1110005' ? 'image' : 'video'}
...
)(withRouter(LectureDataDetailList));
