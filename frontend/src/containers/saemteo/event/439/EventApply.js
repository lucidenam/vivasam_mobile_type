...
class EventApply extends Component {
	...
	applyButtonClick = (target) => {
		...
		try {
			event.eventAnswerDesc = event.inputType + '/' + event.schName + '/' + event.schAddr + ' ' + event.addressDetail + '/' + event.schZipCd +  '/' + event.cellphone;

			// 동반 인원
			event.eventAnswerDesc2 = "동반인원 : " + event.memberCnt + '명';

			//방문희망일
			const tmpDay1 = (day1 === '' ? '' : moment(day1).format("YYYY-MM-DD"));
			const tmpDay2 = (day2 === '' ? '' : moment(day2).format("YYYY-MM-DD"));
			if(tmpDay1 !== '') {
				event.eventAnswerDesc2 += "^||^방문희망일1 : " + tmpDay1;
			}
			if(tmpDay2 !== '') {
				event.eventAnswerDesc2 += "/방문희망일2 : " + tmpDay2;
			}

			//신청이유
			event.eventAnswerDesc2 += "^||^신청이유 : ";

			event.joinReason.forEach((value, i) => {
				if(value === '1'){
					event.eventAnswerDesc2 += "비바샘을 200% 활용 할 수 있는 방법이 궁금해요.";
				}
				if(value === '2'){
					event.eventAnswerDesc2 += "비바샘 개선 및 보완점을 직접 전달하고 싶어요!";
				}
				if(value === '3'){
					event.eventAnswerDesc2 += "주변 선생님께 비바샘을 더 잘 소개해 주세요!";
				}
				if(value === '4'){
					event.eventAnswerDesc2 += "기타";
				}
			...
			//궁금한 콘텐츠 및 페이지 개선
			event.eventAnswerDesc2 += "^||^기타 요청사항 : " + event.applyContent1;
	...
	changeDay2 = (date) => {
		...
		if(date <= day1) {
			common.info("방문 희망일2는 방문 희망일1이후 날짜로 선택이 가능합니다.");
			this.setState({
				day2: ''
			})
			return;
	...
	render() {
		const {eventInfo, ableDayArr} = this.state;
		if (eventInfo === '') return <RenderLoading/>;
		const {event} = this.props;
		const {phoneCheckMessage, phoneCheckClassName, day1, day2, day1CalVisible, day2CalVisible, nowMonth} = this.state;
			...
			<section className="vivasamter event220210">
				...
				<div className="applyDtl_top">
					<div className="applyDtl_cell ta_c pick">
						<h3><strong>비바샘이 간다 신청하기</strong></h3>
					</div>
				</div>
						...
						<div className="pdside20 pb25">
							...
							<h2 className="info_tit tit_flex">
								...
								<div className="person_box type1 ">
									...
									<span className="ml7">명</span>
								</div>
							</h2>


							<h2 className="info_tit mt25">
								<label htmlFor="ipt_visitDay1">방문 희망일 </label>
							</h2>
							<div className="input_wrap calendar_wrap">
								<p className="bulTxt mb15 mt10  type02">* 방문 희망일은 3월 3일 부터 신청가능 하면 신청일로<br /> 부터 최소 1주 후 ~ 최대 2달 이내로 선택 가능(주말 및 공휴일 제외)</p>
								<ul className="pl8">
									<li>
										<span>방문 희망일 ①</span>
										<div className="calendar_box">
											...
											>
												<Calendar
													...
													// tileDisabled={({date, view}) => ableDayArr.find((x) => x === moment(date).format('YYYY-MM-DD'))}
													formatDay={(locale, date) => date.toLocaleString("en", {day: "numeric"})}
													next2Label={null}
													prev2Label={null}
													defaultActiveStartDate={new Date("2023/" + nowMonth + "/01 00:00:00")}
												/>
											</div>
										</div>
									</li>
							...
							<div className="input_wrap reason_list">
								<ul className="pl8">
									<li>
										<input type="checkbox" id="reason1" name="reason" className="checkbox" value="1" onChange={this.setJoinReason} checked={event.joinReason.includes('1')}/>
										<label htmlFor="reason1">비바샘을 200% 활용할 수 있는 방법이 궁금해요.</label>
									</li>
									<li>
										<input type="checkbox" id="reason2" name="reason" className="checkbox" value="2" onChange={this.setJoinReason} checked={event.joinReason.includes('2')}/>
										<label htmlFor="reason2">비바샘 개선 및 보완점을 직접 전달하고 싶어요!</label>
									</li>
									<li>
										<input type="checkbox" id="reason3" name="reason" className="checkbox" value="3" onChange={this.setJoinReason} checked={event.joinReason.includes('3')}/>
										<label htmlFor="reason3">주변 선생님께 비바샘을 더 잘 소개해 주세요!</label>
									</li>
									<li>
										<input type="checkbox" id="reason4" name="reason" className="checkbox" value="4" onChange={this.setJoinReason} checked={event.joinReason.includes('4')}/>
						...
						<div className="acco_notice_list pdside20">
							<div className="acco_notice_cont">
								...
								<ul className="privacyList">
									<li>이용목적 : ‘비바샘이 간다’ 신청 확인​</li>
									<li className="infoIndent">
										수집하는 개인정보 :​ 성명, 학교명, 담당 학년, 교과목,<br />
										비상교과서 채택여부, 휴대전화번호​
									</li>
									<li>
										개인정보 보유 및 이용기간 : 2023년 12월 31일까지<br />
										(이용목적 달성 시 즉시 파기)​
									</li>
								</ul>
								<br />
