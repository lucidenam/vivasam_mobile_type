...
class EventApply extends Component {
	...
	validateInfo = () => {
		...
		if (!event.userName) {
			...
			obj.message = '휴대전화번호를 입력해주세요.';
		} else if (eventAnswer.eventId == 412 && (requestMessage === "" || requestMessage.length === 0)) {
			obj.message = '이유를 입력해 주세요.';
	...
	applyButtonClick = (target) => {
		...
		let comment = eventAnswer.eventId == 412
			? eventAnswer.eventAnswerContent + requestMessage
			: eventAnswer.eventAnswerComment ;

		try {
			event.eventId = eventId;
			event.eventAnswerDesc = receiveInfo;
			event.eventAnswerDesc2 = comment;
			SaemteoActions.pushValues({type: "event", object: event});
			// 신청 처리
	...
	render() {
		const {event, eventAnswer} = this.props;
		const {eventInfo, requestMessage, phoneCheckMessage, phoneCheckClassName} = this.state;
			...
			<section className="vivasamter">
				...
				<div className="applyDtl_top">
					<div className="applyDtl_cell ta_c pick">
						{
							eventAnswer.eventId == 412
								? <h3><strong>비상.한.여름 밤의 꿈 [이벤트 1]</strong></h3>
								: <h3><strong>비상.한.여름 밤의 꿈 [이벤트 2]</strong></h3>
						}
					</div>
				</div>
					...
					<div className="vivasamter_applyDtl pdside0">
						<div className="pdside20 pb25">
							...
							</div>
							{
								eventAnswer.eventId == 412 &&
								<Fragment>
									<h2 className="info_tit info_textarea">
										<label htmlFor="ipt_txt">선택하신 주인공이 선생님과 어울린다고 생각하는 이유는 <br/>
											무엇인가요?
											<div className="txt_count">
												<span><span className="currentCount">{requestMessage.length}</span>/100</span>
											</div>
										</label>
									</h2>
									<div className="input_wrap">
										<textarea placeholder="이유를 적어주세요. (100자 이내)" onChange={this.handleSetComment} value={requestMessage}>
										</textarea>
									</div>
								</Fragment>
							}
						...
						<div className="acco_notice_list pdside20">
							<div className="acco_notice_cont">
								...
								<ul className="privacyList event2">
									...
									<li>개인정보 보유 및 이용기간: 2022년 8월 31일까지<br />(이용목적 달성 시 즉시 파기)</li>
									<li>연락처 오류 시 경품 재발송이 불가능합니다.<br /> 개인정보를 꼭 확인해 주세요.</li>
									<li>선물 발송을 위해 개인정보(성명, 휴대전화번호)가 서비스사와 상품 배송 업체에 제공됩니다.<br/>
										(㈜카카오v 사업자등록번호 120-81-47521)</li>
								</ul>
								<br />
