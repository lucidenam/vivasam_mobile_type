...
class EventApply extends Component {
	...
	componentDidMount() {
		...
		else if (!eventAnswer.selContent || !eventAnswer.eventAnswerDesc) {
	...
	handleSetSchool = (obj) => {
		...
		const { schoolName, schoolCode, zip, addr, tab } = obj;
		if(tab !== 'H'){
	...
	validateInfo = () => {
		...
		const {telephoneCheck, peopleCnt} = this.state;
	...
	applyButtonClick = (target) => {
		...
		try {
			...
			event.eventAnswerDesc = event.inputType + '/' + event.schName + '/' + event.cellphone + '/' + event.schZipCd + '/' + event.schAddr + ' ' + event.addressDetail
				+ '/수령처 : ' + receive + '/학급 인원수 : '+this.state.peopleCnt;
			SaemteoActions.pushValues({type: "event", object: event});
			// 신청 처리
	...
	insertApplyForm = async () => {
		...
		const rdoCheckedType = Number(eventAnswer.rdoChecked);

		try {
			if (eventAnswer.selContent == '' || eventAnswer.eventAnswerDesc == '') {
				throw new Error("선택한 정보가 없습니다. 다시 신청해 주세요.");
			}
			...
			var params = {
				...
				eventAnswerDesc2: eventAnswer.selContent + '^||^' + eventAnswer.eventAnswerDesc,
		...
		try{
			if(Number(e.target.value)){
				if(Number(e.target.value) < 1 || Number(e.target.value) > 30){
					common.info('학급 인원수는\n1명부터 최대 30명까지 입력 가능합니다.');
					this.setState({
						peopleCnt: ''
					});
				}else{
					this.setState({
						peopleCnt: e.target.value
					});
				}
			}
		}catch(ex){
	...
	render() {
		...
		const {event} = this.props;
		const {phoneCheckMessage, phoneCheckClassName, peopleCnt} = this.state;
				
		return (
			<section className="vivasamter event220321">
				<h2 className="blind">
					비바샘터 신청하기
				</h2>
				<div className="applyDtl_top">
					<div className="applyDtl_cell ta_c pick">
						<h3><strong>Full수록 반배송 이벤트</strong></h3>
					</div>
				</div>
				<div className="vivasamter_apply">
					<div className="vivasamter_applyDtl pdside0">
						<div className="pdside20 pb25">
							...
							{event.userInfo == 'Y' && <p className="bulTxt mb25 mt10 ml10">* 학교 검색에서 찾으시는 학교가 없을 경우,<br/><span className="ml8">직접 입력을 통해 재직학교와 주소를 입력해 주세요.</span></p>}
							<h2 className="info_tit">
							...
							<h2 className="info_tit mt25">
								<label htmlFor="ipt_peopleCnt">학급 인원 수</label>
							</h2>
							<div className='input_wrap receiveGradeClass'>
								<input
									...
									maxLength="2"
									name="peopleCnt"
									onChange={this.setPeopleCnt}
									className="input_sm"
									value={peopleCnt}
								/>
								<span className="label_txt">명</span>
							</div>
							<p class="pointColor mb25 mt10 ml10">* 최대 30명까지 입력 가능</p>
							<h2 className="info_tit">
						...
						<div className="acco_notice_list pdside20">
							<div className="acco_notice_cont">
								...
								<ul className="privacyList">
									...
									<li>선물 발송을 위해 배송/서비스 업체에 개인정보(성명, 학교주소, 휴대전화번호)가 제공됩니다.<br />(㈜CJ대한통운 사업자번호 110-81-05034)<br />((주)모바일이앤엠애드 사업자번호: 215-87-19169)</li>
								</ul>
								<br />
								<p className="privacyTxt">선생님께서는 개인정보의 수집 및 이용, 취급 위탁에 대한 동의를 거부할 수 있습니다.<br />단, 동의를 거부할 경우 이벤트 참여가 불가합니다.</p>
							</div>
						</div>
