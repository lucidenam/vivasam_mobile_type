...
class EventApply extends Component {
    ...
    handleChange = (e) => {
        ...
        }else if(e.target.name === 'ipt_quantity'){
            this. state.amount1 = e.target.value;
        }else if(e.target.name === 'ipt_quantity2'){
            this. state.amount2 = e.target.value;
        }else{
    ...
    applyButtonClick = async(target) => {
        ...
        if(this.state.amount1 > 0) {
            ...
            if (chkQntCnt < this.state.amount1) { // 해당된 수량만큼 제한
                common.error("우리는 모두 친구! 신청가능 잔여 수량은 " + chkQntCnt + "부 입니다.");
        ...
        if(this.state.amount2 > 0) {
            ...
            if (chkQntCnt < this.state.amount2) { // 해당된 수량만큼 제한
                common.error("너의 꿈은 꼭 이루어질거야! 신청가능 잔여 수량은 " + chkQntCnt + "부 입니다.");
                target.disabled = false;
                return false;
            }
        }
        ...
        try {
            ...
            event.eventAnswerDesc = event.inputType + '/' +event.schName + '/' +event.cellphone + '/' +event.schZipCd + '/' +event.schAddr + ' ' +event.addressDetail + '/수령처 : ' + receive + '/1. 우리는 모두 친구! : '
                + this.state.amount1 + '부 , 2. 너의 꿈은 꼭 이루어질거야! : ' + this.state.amount2 + '부';
            SaemteoActions.pushValues({type:"event", object:event});
    ...
    insertApplyForm = async () => {
        ...
        try {
            ...
            if(response.data.code === '1'){
                ...
                event.eventAnswerDesc = this.state.amount1; // 사연
                event.eventAnswerSeq = 2;
                response = await SaemteoActions.setEventJoinAnswerAddInsert({...event});
                event.eventAnswerDesc = this.state.amount2; // 사연
            ...
            <section className="vivasamter">
                <h2 className="blind">
                    ...
                    <div className="applyDtl_cell">
                        <h3> 비상교육 <em>항균 미니 포스터</em> 신청하기</h3>
                    </div>
                </div>
                    ...
                    <div className="vivasamter_applyDtl">
                        ...
                        <div className="quantity_wrap">
                            <div className="input_wrap mb5">
                                <label htmlFor="ipt_quantity">① 우리는 모두 친구!</label>
                            ...
                            <div className="input_wrap mb15">
                                <label htmlFor="ipt_quantity2">② 너의 꿈은 꼭 이루어질 거야!</label>
                                <div className="selectbox select_sm">
                                    <select
                                        name="ipt_quantity2"
                                        id="ipt_quantity2"
                            ...
                            <div className="input_wrap mb40">
                                <ul>
                                    ...
                                    <li>* 1종에 최대 5매씩 신청 가능합니다. (2종 총 10매까지 신청 가능)</li>
                                    <li>* 6월 10일부터 순차적으로 발송됩니다.</li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div className="acco_notice_list">
                            <div className="acco_notice_cont">
                                ...
                                <ul className="policy">
                                    <li>&middot; 이용목적 : 포스터 발송</li>
                                    <li>&middot; 수집하는 개인정보 : 성명, 학교명, 학교 주소, 연락처
                                    </li>
                                    <li>&middot; 개인정보 보유 및 이용기간 : 2020년 6월 30일까지<br/>
                                        (이용목적 달성 시 즉시 파기)
                                    </li>
                                    <li>&middot; 수집하는 개인정보의 취급위탁 : 포스터 발송을 위해 개인정보<br/>
                                        (이름/주소/연락처)를 배송업체에 취급 위탁<br/>
