...
class EventApply extends Component {
	...
	componentDidMount() {
		...
		//  선택된 이벤트가 없을 경우 이전 페이지로 되돌림
		if(!eventId) {
			history.push('/saemteo/event');
		} 
		else if (!eventAnswer.activeSubEventId) {
	...
	applyButtonClick = (target) => {
		...
		try {
			...
			event.eventAnswerDesc2 = eventAnswer.giftName + '^||^' + eventAnswer.nickname + '/' + eventAnswer.message;
	...
	insertApplyForm = async () => {
		const {event, history, SaemteoActions, PopupActions, BaseActions, MyclassActions, eventAnswer, eventId} = this.props;
		
		try {
			BaseActions.openLoading();
			
			var selectedGiftCountArr = [];
			// 상품수 5개, 상품신청수량을 배열로 정리
			for (let i=0, size=5; i<size; i++) {
				if (eventAnswer.giftNoIdx === i) {
					selectedGiftCountArr.push(1);
				} else {
					selectedGiftCountArr.push(0);
				}
			}
			
			const params = {
				cellphone: event.cellphone,
				eventId : eventAnswer.activeSubEventId,
				...
				amountYn: 'Y',   /* 수량제한 신청 */
				applyContentTotCnt: '5',     /* 상품 종류 수 */
				applyContentNumbers: '3,4,5,6,7',    /* 상품 seq, 복수신청시는 csv(3,4,5) */
				applyTargetContentCnt: selectedGiftCountArr.join(',')
			};

			
			// 이벤트 신청전 이벤트 선물 잔여수량을 체크 - 신청서 작성중에 마감될 수 있음
			const params1 = {};
			params1.eventId = eventAnswer.activeSubEventId; // 이벤트 ID
			try {
				const response = await SaemteoActions.chkEventRemainsQntCnt({...params1});
				const responseData = response.data;
				const remains = responseData['qntCnt_'+eventAnswer.giftNo];
			...
			if (response.data.code === '1') {
				...
				PopupActions.openPopup({title:"신청완료", componet:<EventApplyResult eventId={event.eventId} handleClose={this.handleClose}/>});
						...
						<div className="acco_notice_list pdside20">
							<div className="acco_notice_cont">
								...
								<ul className="privacyList">
									...
									<li>수집하는 개인정보 : 성명, 학교명, 학교주소, 휴대폰 번호</li>
									<li>개인정보 보유 및 이용기간: 2021년 9월 30일까지<br />(이용목적 달성 시 즉시 파기)</li>
									<li>연락처 오류 시 경품 재발송이 불가능합니다.<br />개인정보를 꼭 확인해 주세요.</li>
									<li>선물 발송을 위해 개인정보(성명, 학교주소, 휴대폰 번호)가 배송업체에 제공됩니다.<br />((주)한진택배 사업자등록번호 201-81-02823)</li>
								</ul>
								<br />
