...
class EventApply extends Component {
	...
	handleChange = (e) => {
		...
		const {tempAgree1, tempAgree2} = this.state;

		if (e.target.name === 'agree1' || e.target.name === 'agree2') {
			if (e.target.name === 'agree1') {
				this.setState({
					tempAgree1: !tempAgree1
				})

				event[e.target.name] = tempAgree1;
			} else {
				this.setState({
					tempAgree2: !tempAgree2
				})

				event[e.target.name] = tempAgree2;
			}
		} else {
	...
	handleUserInfo = (e) => {
		...
		SaemteoActions.pushValues({type: "event", object: event});
		this.handleChange(e);
		// this.handleChangeRecommender(e);
	...
	validateInfo = () => {
		...
		const {telephoneCheck, tempAgree1,tempAgree2} = this.state;
		...
		if (!event.schName) {
			...
			obj.message = '휴대전화번호를 입력해 주세요.';
		} else if (eventAnswer.eventId == 544 && event.receive === "") {
			obj.message = '수령처를 선택해주세요.';
		} else if (eventAnswer.eventId == 544 && event.receive === "교실" && (event.receiveGrade === "" || event.receiveClass === "")) {
			obj.message = '학년 반을 입력해주세요.';
		} else if (eventAnswer.eventId == 544 && event.receive === "기타" && event.receiveEtc === "") {
			obj.message = '수령처를 입력해주세요.';
		} else if (eventAnswer.eventId == 544 && !tempAgree1) {
			obj.message = '필수 동의 선택 후 이벤트 신청을 완료해 주세요.';
		} else if (eventAnswer.eventId == 545 && !tempAgree2) {
			obj.message = '필수 동의 선택 후 이벤트 신청을 완료해 주세요.';
		} else {
			obj.result = true;
		}
	...
	render() {
		const {eventInfo, tempAgree1, tempAgree2} = this.state;
					...
					eventAnswer.eventId === 544
						...
						<Fragment>
							...
							<div className="applyDtl_top ">
								<div className="applyDtl_cell ta_c pick">
									<h3>
										<strong>EVENT 2 - </strong> 신규 회원가입 이벤트
									</h3>
								</div>
							</div>
							<div className="vivasamter_apply">
								<div className="vivasamter_applyDtl type02 pdside0 ">
									<div className="apply_form_wrap pdside20 pb15">
										...
										<div
											className={'combo_box ' + (event.receive === '교실' ? 'type2' : (event.receive === '기타' ? 'type3' : 'type1'))}>
											...
											<div
												className={'input_wrap mt5 receiveEtc ' + (event.receive === '기타' ? '' : 'hide')}>
												<input
													type="text"
													autoCapitalize="none"
													name="receiveEtc"
									...
									<div
										className={"apply_noti noti2 " + (event.recommendInfo == 'Y' ? "on " : " ") + (event.recommendInfo && idCheckOn ? "on2" : " ")}>
										<p>
									...
									<div className="acco_notice_list mt0">
										<div className="acco_notice_cont">
											...
											<ul className="privacyList type02 ">
												...
												<li>수집하는 개인 정보: 성명, 재직 학교, 휴대전화번호, 학교 주소, 자택 주소</li>
												<li>개인정보 보유 및 이용 기간: 2025년 6월 30일까지<br/>(이용목적 달성 시 즉시 파기)</li>
												<li>개인정보 오기재로 인한 경품 재발송은 불가능합니다.<br/>개인정보를 꼭 확인해 주세요.</li>
												<li>경품 발송을 위한 개인정보(성함, 휴대 전화번호, 주소)가 배송업체 및 기프티콘 발송 서비스사에 제공됩니다.<br/>
													(㈜카카오 사업자등록번호 : 120-81-47521)/<br/>(㈜모바일이앤엠애드
													사업자등록번호:215-87-19169)<br/>(CJ대한통운(주) 사업자등록번호 : 110-81-05034)
												</li>
											</ul>
											<br/>
									...
									<div className="checkbox_circle_box acco_notice_list notice_sec">
										<div className="acco_notice_cont">
											<input
												type="checkbox"
												name="agree1"
						...
						<Fragment>
							...
							<div className="applyDtl_top">
								<div className="applyDtl_cell ta_c pick">
									<h3>
										<strong>EVENT 03 - </strong> 동료 선생님 추천 이벤트
									</h3>
								</div>
							</div>
							<div className="vivasamter_apply">
