...
class Event extends Component{
	...
	eventAmountCheck = async() => {
		...
		//검수용
		/*
		if( allAmountFull || (DD == 11 && todayAmountFullArr[0]) || (DD == 12 && todayAmountFullArr[1]) || (DD == 13 && todayAmountFullArr[2]) || (DD == 14 && todayAmountFullArr[3]) || (DD == 15 && todayAmountFullArr[4])) {
			this.setState({todayItemEnd: true});
		}
		*/
		//실서버용
		if( allAmountFull || (DD == 18 && todayAmountFullArr[0]) || (DD == 19 && todayAmountFullArr[1]) || (DD == 20 && todayAmountFullArr[2]) || (DD == 21 && todayAmountFullArr[3]) || (DD == 22 && todayAmountFullArr[4])) {
			this.setState({todayItemEnd: true});
		}
	...
	eventApply = async () => {
		...
		// 이벤트 기간 18~22일 : 기간별로 상품 3~6번 차례로 지급
		// 18일 이전은 테스트 용도

		//검수용
		/*
		if( allAmountFull || (DD == 11 && todayAmountFull[0]) || (DD == 12 && todayAmountFull[1]) || (DD == 13 && todayAmountFull[2]) || (DD == 14 && todayAmountFull[3]) || (DD == 15 && todayAmountFull[4])) {
			if(DD == 11 || DD == 12 || DD == 13 || DD == 14) {
		...
		*/
		//실서버용
		if( allAmountFull || (DD == 18 && todayAmountFull[0]) || (DD == 19 && todayAmountFull[1]) || (DD == 20 && todayAmountFull[2]) || (DD == 21 && todayAmountFull[3]) || (DD == 22 && todayAmountFull[4])) {
			if(DD == 18 || DD == 19 || DD == 20 || DD == 21) {
				common.info("오늘 준비된 수첩과 달력이 모두 소진되어 신청이 마감되었습니다. \n 내일 오후 4시에 다시 신청해 주세요:)");
			}
			if(DD == 22) {
				common.info("준비한 다이어리와 달력이 모두 소진되어 \n 신청이 마감되었습니다. 감사합니다!");
	...
	render () {
		...
		return (
			<section className="event241118">
				<div className="evtCont1">
					<div className="evtTit">
						<img src="/images/events/2024/event241118/img.png" alt="비바샘과 미리 만나보는 2025"/>
						<span className={this.state.todayItemEnd ? "item_end deadline" : "item_end"}>{/*마감 시 deadline 클랙스 추가해주세요*/}
							<img src="/images/events/2024/event241118/evtEnd.png" alt="금일 선착순 수량 마감"/>
				...
				<div className="evtCont02">
					<div className="item_box">
						<img src="/images/events/2024/event241118/item.png" alt="2025 선생님 다이어리, 2025 선생님 탁상달력"/>
				...
				<div className="evtNotice">
					<strong>유의사항</strong>
					<ul className="info">
						<li>1. 본 이벤트는 비바샘 교사인증을 완료한 <span>초중고 학교 선생님</span>만 신청하실 수 있습니다.</li>
						<li>2. <span>이벤트 기간 내</span> 1인 1회 신청 가능합니다. <span>(중복 신청 불가)</span></li>
						<li>3. 매일 선착순 신청으로, 수량 소진 시 이벤트가 마감됩니다.</li>
						<li>4. 다이어리와 탁상 달력은 학교로만 배송이 가능합니다. 학교 주소와 수령처를 정확하게 기입해 주세요.</li>
						<li>5. 선물 발송에 필요한 정보는 배송 업체에 공유됩니다. <br/>(성명, 주소, 전화번호 등/ CJ대한통운(주) 사업자등록번호 : 110-81-05034)</li>
						<li>6. 주소 기재가 잘못되어 오발송된 물품은 재발송해 드리지 않습니다.</li>
					</ul>
				</div>
			</section>
		)
