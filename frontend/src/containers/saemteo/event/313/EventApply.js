...
class EventApply extends Component {
    ...
    componentDidMount(){
        ...
        if((eventAnswer.eventAnswerContent.selWordData == null) || (typeof eventAnswer.eventAnswerContent.selWordData == "undefined")
    ...
    applyButtonClick = (target) => {
        ...
        try {
            ...
            if(
                (eventAnswer.eventAnswerContent.selWordData.length == 0) || (eventId == "")
            ){
            ...
            for(let i=0; i<eventAnswer.eventAnswerContent.selWordData.length; i++){
                if(i==0){
                    selectContentInfo = eventAnswer.eventAnswerContent.selWordData[i].word + '@' + eventAnswer.eventAnswerContent.selWordData[i].applyContent;
                }else{
                    selectContentInfo += ' , '+eventAnswer.eventAnswerContent.selWordData[i].word + '@' + eventAnswer.eventAnswerContent.selWordData[i].applyContent;
                }
            }
            event.selectContentInfo = selectContentInfo;
            event.eventAnswerDesc = event.inputType + '/' +event.schName + '/' +event.cellphone + '/' +event.schZipCd + '/' +event.schAddr + ' ' +event.addressDetail + '/수령처 : ' + receive + '/선택낱말 : '+selectContentInfo;
            SaemteoActions.pushValues({type:"event", object:event});
            // 신청 처리
            ...
            <section className="vivasamter">
                ...
                <div className="applyDtl_top">
                    <div className="applyDtl_cell ta_c pick">
                        <h3><strong>학교 낱말 사전</strong> 참여하기</h3>
                    </div>
                </div>
                <div className="vivasamter_apply">
                    <div className="vivasamter_applyDtl">
                        ...
                        <p className="mt10 mb25 c_gray">
                            * 경품은 학교로 발송됩니다. 학교 소재지를 정확히 입력해 주세요.<br />
                            * 주소 오류로 반송된 경품을 재발송이 불가능합니다.
                        </p>
                        <div className="acco_notice_list">
                            ...
                            <div className="acco_notice_cont mt10">
                                <ul class="policy">
                                    ...
                                    <li>•수집하는 개인정보 : 성명, 재직학교, 학교 소재지, 휴대전화번호</li>
                                    <li>•개인정보 보유 및 이용기간 : 2020년 12월 31일까지 <br/>(이용목적 달성 시 즉시 파기)</li>
                                    <li>•수집하는 개인정보의 취급위탁 : 선물 발송을 위해 개인정보를 배송업체에 취급 위탁 (㈜한진 사업자등록번호:201-81-02823)</li>
                                </ul>
                        ...
                        <button
                            ...
                            className="btn_full_on2 mt35">신청완료</button>
                    </div>
                </div>
            </section>
