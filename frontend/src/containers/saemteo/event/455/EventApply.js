...
class EventApply extends Component {
	...
	componentDidMount() {
		...
		// this.getEventInfo(eventId);
		// console.log("eventAnswer", eventAnswer.eventContent);
	...
	validateInfo = () => {
		...
		const {telephoneCheck, requestMessage} = this.state;
	...
	applyButtonClick = (target) => {
		...
		// let receiveInfo = event.inputType + '/' + event.schName + '/' + event.emailId + '@' + event.emailDomain + '/' + event.cellphone+ '/' + event.schAddr + event.addressDetail + '/수령처 : ' + receive + "/인원 : " + this.state.isSetMemberCount;
		let receiveInfo = event.inputType + '/' + event.schName + '/' + this.state.eMailDomain + '@' + this.state.anotherEmailDomain + '/' + event.cellphone+ '/' + event.schAddr + event.addressDetail + '/수령처 : ' + receive + "/인원 : " + this.state.isSetMemberCount;

		let answer = eventAnswer.eventContent;
	...
	render() {
		const {event, eventAnswer} = this.props;
		const {eventInfo, requestMessage, phoneCheckMessage, phoneCheckClassName, eMailDomain, anotherEmailDomain} = this.state;
				...
				<div className="vivasamter_apply">
					<div className="vivasamter_applyDtl pdside0">
						<div className="pdside20 pb25">
							...
							<div className="selectbox select_sm mt5 mb10">
							...
							<h2 className="info_tit">
								<label htmlFor="personnel">학급 인원</label>
							</h2>
							<div className="input_wrap multi_wrap">
								<div className="input_wrap">
									<input
										type="number"
										name="personnel"
										...
										onKeyPress={(e) => {
											...
											if (/[ㄱ-ㅎㅏ-ㅣ가-힣]/.test(keyValue) || /[^0-9]/.test(keyValue)) {
												e.preventDefault();
											}
										}}
										maxLength={3}
										className="input_sm"/>
									<span className="label_txt ml5">명</span>
								</div>
							</div>
						</div>
						<div className="acco_notice_list pdside20">
							<div className="acco_notice_cont">
								...
								<ul className="privacyList event2">
									<li>이용목적: 경품발송 및 고객 문의 응대​</li>
									<li>수집하는 개인정보: 성명, 학교명, 학교 주소, 담당 학년, 이메일, 휴대전화번호</li>
									<li>개인정보 보유 및 이용기간: 2023년 8월 31일까지<br />(이용목적 달성 시 즉시 파기)</li>
									<li>
										경품 발송을 위해 개인정보(성명, 주소, 연락처)가 배송업체에<br/>
										(제공됩니다.(㈜카카오 사업자 등록번호 120-81-47521)
									</li>
								</ul>
