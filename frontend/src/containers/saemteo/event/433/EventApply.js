...
class EventApply extends Component {
	...
	handleChange = (e) => {
		...
		if(e.target.name == 'youTubeJoin' && e.target.value === 'N') {
			event.youTubeId = '';
		}
	...
	validateInfo = () => {
		...
		if (!event.userName) {
			...
			obj.message = '휴대전화번호를 입력해주세요.';
		} else if (event.youTubeJoin != 'Y' && event.youTubeJoin != 'N' ) {
			obj.message = '유튜브 구독&댓글 참여 여부를 선택해 주세요​.';
		} else if (event.youTubeJoin == 'Y' && event.youTubeId === "") {
			obj.message = '선생님의 유튜브 계정을 입력해주세요.';
		} else if (!event.agree) {
	...
	applyButtonClick = (target) => {
		...
		let answer = event.teacherAnnual + '^||^' + event.teacherHope + '^||^';
		if(event.youTubeJoin == 'Y') {
			answer += '유튜브 참여/id : ' + event.youTubeId;
		} else {
			answer += '유튜브 미참여';
		}
			...
			<section className="vivasamter">
				...
				<div className="applyDtl_top top_yell">
					<div className="applyDtl_cell ta_c pick">
						<h3><strong><span>2023년 선생님 행복 ON 이벤트</span> 신청하기</strong></h3>
					</div>
				</div>
				<div className="vivasamter_apply">
					<div className="vivasamter_applyDtl pdside0">
						<div className="pdside20 pb25">
							...
							<h2 className="info_tit tit_flex" id="ipt_ytJoin">
								<label htmlFor="ipt_ytJoin">유튜브 구독&#38;댓글 참여 여부</label>
								<ul className="join_ipt_chk">
									<li className="join_chk_list half">
										<input
											id="ytJoinY"
											...
											name="youTubeJoin"
											value="Y"
											checked={event.youTubeJoin === 'Y'}
										/>
										<label htmlFor="ytJoinY">참여</label>
									</li>
									<li className="join_chk_list half">
										<input
											id="ytJoinN"
											...
											name="youTubeJoin"
											value="N"
											checked={event.youTubeJoin === 'N'}
										/>
										<label htmlFor="ytJoinN">미참여</label>
									</li>
								</ul>
							</h2>
							<h2 className="info_tit">
								<label htmlFor="ipt_school_name">선생님 유튜브 계정</label>
							</h2>
							<div className="input_wrap school">
								<input
									type="text"
									placeholder="예) vivasam_official"
									id="ipt_ytId"
									name="youTubeId"
									onChange={this.handleChange}
									value={event.youTubeId}
									className="input_sm"
									disabled={event.youTubeJoin === 'N'}
								/>
							</div>
						</div>
						<div className="acco_notice_list pdside20 ">
							<div className="acco_notice_cont">
								...
								<ul className="privacyList event2">
									<li>개인정보 수집 및 이용 동의 이용 목적 : 경품 발송 및 고객문의<br /> 응대</li>
									<li>집하는 개인정보 : 성명, 재직학교, 주소, 휴대전화번호, 유튜브 <br /> 계정</li>
									<li>
										폴라로이드 경품의 경우,제세공과금(22%)이 부여되며 <br /> 제세공과금 처리를 위한
									개인정보 수급 (신분증 사본, <br /> 주민등록번호, 계좌번호)을 위해 별도로 안내를 드릴 예정입니다.
									</li>
									<li>개인정보 보유 및 이용기간 : 2023년 5월 30일까지 (이용목적 <br /> 달성 시 즉시 파기)</li>
									<li>연락처 오류 시 경품 재발송이 불가능합니다. 개인정보를 꼭 <br /> 확인해 주세요.</li>
									<li>경품(폴라로이드, 학급간식)은 재직하고 계신 학교 택배함 또는<br /> 교무실로 배송됩니다.</li>
									<li>
										이벤트 당첨자의 잘못된 개인정보 전달로 인한 불이익(연락 불가,<br /> 경품 반송/분실 등)은 <br />
										책임지지 않습니다
									</li>
									<li>
										선물 발송을 위해 개인정보(성명, 휴대전화번호)가 서비스사와<br /> 배송업체에 제공됩니다.<br />
