...
class EventApply extends Component {
	...
	getEventInfo = async (eventId) => {
		...
		if (response.data.code && response.data.code === "0") {
			...
			event.addressDetail = ''; // 이번 이벤트에서만 빈값으로
			event.inputType = '학교';
			event.userInfo = 'Y';
	...
	render() {
		...
		return (
			<section className="vivasamter event240819">
				<h2 className="blind">
					비바샘터 신청하기
				</h2>
				<div className="applyDtl_top bg_pink">
					<div className="applyDtl_cell ta_c pick">
						<h3>디지털 교육혁신 시나리오 카드 배포 이벤트 신청하기</h3>
					</div>
				</div>
							...
							<h2 className="info_tit">
								...
								<ul className="join_ipt_chk">
									<li className="join_chk_list half ml38">
										...
										/>
										<label htmlFor="userInfoY">학교</label>
									</li>
