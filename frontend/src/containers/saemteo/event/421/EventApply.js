...
class EventApply extends Component {
	...
	applyButtonClick = (target) => {
		...
		let questionList = '';
		let tmpQuestionList = eventAnswer.eventAnswerContent;
		tmpQuestionList.map((arr, index) => (
			(index + 1) === eventAnswer.eventAnswerContent.length ?
			questionList += arr.name + '번: ' + arr.value : questionList += arr.name + '번: ' + arr.value + ', '
		))
		...
		try {
			...
			event.eventAnswerDesc2 = questionList;
			SaemteoActions.pushValues({type: "event", object: event});
			// 신청 처리
			...
			<section className="vivasamter">
				...
				<div className="applyDtl_top bg_yell">
					<div className="applyDtl_cell ta_c pick">
						<h3><strong>VISANG + 한해 7탄 비상한 모의고사</strong></h3>
					</div>
				</div>
					...
					<div className="vivasamter_applyDtl pdside0">
						...
						<div className="acco_notice_list pdside20">
							<div className="acco_notice_cont">
								...
								<ul className="privacyList">
									...
									<li>개인정보 보유 및 이용기간 : 2022년 12월 31일까지<br/>
										(이용목적 달성 시 즉시 파기)</li>
									<li>
										연락처 오류 시 경품 재발송이 불가능합니다.<br/>
										개인정보를 꼭 확인해 주세요.<br/>
										선물 발송을 위해 서비스/배송 업체에 개인정보 (성명, 학교주소, <br/>
										휴대전화번호)가 제공됩니다.<br />
										(㈜카카오 사업자등록번호 120-81-47521,<br />
										㈜한진 사업자등록번호 : 201-81-02823,<br />
										㈜CJ대한통운 사업자번호 :  110-81-05034)
									</li>
						...
						<div className="checkbox_circle_box mt25 pdside20">
							<input
								...
								className="checkbox_circle checkbox_circle_rel evt_agree"
								id="join_agree01"/>
							<label
