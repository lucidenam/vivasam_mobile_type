...
class EventApply extends Component {
	...
	validateInfo = () => {
		...
		} else if (eventAnswer.eventId === 550 && event.receive === "") {
			obj.message = '수령처를 선택해주세요.';
		} else if (eventAnswer.eventId === 550 && event.receive === "교실" && (event.receiveGrade === "" || event.receiveClass === "")) {
			obj.message = '학년 반을 입력해주세요.';
		} else if (eventAnswer.eventId === 550 && event.receive === "기타" && event.receiveEtc === "") {
			obj.message = '수령처를 입력해주세요.';
		} else if (eventAnswer.eventId === 550 && !event.receiveEtc2) {
			obj.message = '학급 학생 수를 입력해주세요.';
		} else if (eventAnswer.eventId === 550 && this.state.eventContents === "") { // 내용 미입력
	...
	applyButtonClick = (target) => {
		...
		if (eventAnswer.eventId == 549) {
			receiveInfo = userInfo + '/' + event.schName + '/' + event.schZipCd + '/' + event.schAddr + '/' + event.addressDetail + '/' + event.cellphone;
		}
		if (eventAnswer.eventId == 550) {
			receiveInfo = userInfo + '/' + event.schName + '/' + event.schZipCd + '/' + event.schAddr + '/' + event.addressDetail + '/' + event.cellphone + '/ 학생 수 : ' + event.receiveEtc2 + '/수령처 : '+receive;
			...
			<section className="vivasamter">
				{
					eventAnswer.eventId === 549
							...
							<Fragment>
								...
								<div className="applyDtl_top top_yell topStyle2">
									<div className="applyDtl_cell ta_c pick color2">
										<h3>이벤트 1) 똑똑, 간편하게 클래스 문을 열어요</h3>
									</div>
								</div>
										...
										<div className="acco_notice_list pdside20">
											<div className="acco_notice_cont">
												...
												<ul className="privacyList">
													...
													<li>수집하는 개인정보 : 성명, 재직 학교, 휴대전화번호, 학교 주소<br/>개인 정보 보유 및 이용 기간: 2025년 4월 30일까지(이용 목적 달성 시 즉시 파기)</li>
													<li>개인 정보 오기로 인한 경품 재발송은 불가능합니다. 개인 정보를 꼭 확인해 주세요.</li>
													<li>경품 발송을 위해 개인 정보가 서비스사와 배송 업체에 제공됩니다.<br/>(주)모바일이앤엠애드 사업자등록번호 : 215-87-19169</li>
						...
						<Fragment>
							...
							<div className="applyDtl_top top_yell topStyle2">
								<div className="applyDtl_cell ta_c pick color2">
									<h3>이벤트 2) 더욱 똑똑해질 비바클래스에게 한 마디!</h3>
								</div>
							</div>
							<div className="vivasamter_apply">
								<div className="vivasamter_applyDtl pdside0 type02">
									...
									<div className="acco_notice_list pdside20">
										<div className="acco_notice_cont">
											...
											<ul className="privacyList">
												<li>이용목적 : 경품 발송 및 고객 문의 응대</li>
												<li>수집하는 개인정보 : 성명, 재직 학교, 휴대전화번호, 학교 주소, 수령처<br/>※ 학교 주소와 학급 학생 수, 수령처는 이벤트 2 당첨 시 상품을 발송하기 위해 수집합니다.</li>
												<li>개인 정보 보유 및 이용 기간: 2025년 4월 30일까지(이용 목적 달성 시 즉시 파기)</li>
												<li>개인 정보 오기로 인한 경품 재발송은 불가능합니다. 개인 정보를 꼭 확인해 주세요.</li>
												<li>경품 발송을 위해 개인 정보가 서비스사와 배송 업체에 제공됩니다.<br/>(주)모바일이앤엠애드 사업자등록번호 : 215-87-19169 / 아기자기 선물가게 사업자등록번호: 530-31-00427</li>
