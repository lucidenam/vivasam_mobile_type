...
class EventApply extends Component {
    ...
    eventAmountCheck = async() => {
        ...
        try {
            ...
            if (chkQntCnt >= 30) { // 해당된 수량만큼 제한
                this.setState({requestAmountLimit: 30});
            } else {
                this.setState({requestAmountLimit: chkQntCnt});
            }
        } catch (e) {
            history.push('/saemteo/event/view/'+eventId);
        }
    ...
    applyButtonClick = (target) => {
        ...
        try {
            event.requestAmount = this.state.amount;
            if (event.requestAmount <= 0) {
                common.error('최소 신청수량은 1권이상입니다.');
                return false;
            } else if (event.requestAmount > 30) {
                common.error('최대 30권까지 신청 가능합니다.');
                return false;
            }
    ...
    insertApplyForm = async () => {
        const { event, history, SaemteoActions, PopupActions, BaseActions, eventId } = this.props;

        try {
            BaseActions.openLoading();
            event.eventId = eventId;
            event.amountYn = 'Y';
            event.eventAnswerDesc2 = event.requestAmount;
            event.applyContentTotCnt = '1';
            event.applyContentNumbers = '3';
            event.applyTargetContentCnt = event.requestAmount;
    ...
    handleBlurAmount = (e) => {
        ...
        if (amount === '' || amountNum <= 1) {
            amount = '1';
        } else if (amountNum >= this.state.requestAmountLimit) {
            amount = String(this.state.requestAmountLimit);
        }
        this.setState({
            amount : amount
        });
    ...
    handleClickIncrease = () => {
        // 최대수량 30개
        const amountNum = Number(this.state.amount);
        if (amountNum >= this.state.requestAmountLimit) {
            common.info('현재 신청 가능하신 최대 수량입니다.');
            return;
        }
            ...
            <section className="vivasamter">
                ...
                <div className="vivasamter_apply">
                    <div className="vivasamter_applyDtl">
                        ...
                        <h2 className="info_tit">
                            <label htmlFor="ipt_planner_num">2021 스터디 플래너 신청 수량</label>
                        </h2>
                        <div className="input_wrap planner_wrap">
                            <button type="button" onClick={this.handleClickDecrease}>-</button>
                            <input
                                type="number"
                                id="ipt_planner_num"
                                className="input_sm mb5"
                            ...
                            />
                            <button type="button" onClick={this.handleClickIncrease}>+</button>
                        </div>
                        <p className="mb25 pointColor">
                            * 최대 30권까지 신청 가능합니다.
                        </p>
                        <div className="acco_notice_list">
                            <div className="acco_notice_cont mt10">
                                ...
                                <ul className="policy">
                                    <li>- 이용목적 : 이벤트 선물 발송 및 CS 응대</li>
                                    <li>- 수집하는 개인정보 : 성명, 학교정보, 학교 주소, 수령처, 휴대전화번호</li>
                                    <li>- 개인정보 보유 및 이용기간 : 2021년 3월 31일까지<br />(이용목적 달성 시 즉시 파기)</li>
