...
class EventApply extends Component {
	...
	state = {
		...
		requestAmount: '',			// 학급 인원 수
		requestMessage: '',			// 스트랩 메시지
		requestMessageLength: 0,	// 스트랩 메시지 길이
	};
	...
	// 이벤트2 입력
	handleChangeEvent2 = (e) => {
		let comment = e.target.value;
		let commentLength = comment.length;

		if (e.target.name === 'amount') {
			this.setState({
				requestAmount: comment,
			});
		} else {
			if (commentLength >= 20) {
			...
			this.setState({
				...
				requestMessageLength: commentLength
			});
		}
	};
	...
	validateInfo = () => {
		...
		const {telephoneCheck, requestAmount, requestMessage, requestMessageLength} = this.state;
		...
		} else if (eventAnswer.checkEvent2) {
			if (requestAmount == 0 || requestAmount.length <= 0) {
				obj.message = '학급 인원 수를 입력해주세요.';
			} else if (requestMessage.trim() === '' || requestMessageLength <= 0) {
				obj.message = '스트랩 메시지를 입력해주세요.';
			} else {
				obj.result = true;
			}
		} else {
	...
	applyButtonClick = (target) => {
		...
		const {requestAmount, requestMessage} = this.state;
		...
		let comment1 = eventAnswer.eventAnswerDesc ? eventAnswer.eventAnswerDesc : '미참여';
		let comment2 = eventAnswer.checkEvent2 ? requestAmount : '미참여';

		try {
			event.eventId = eventId;
			event.eventAnswerDesc = receiveInfo;
			event.eventAnswerDesc2 = comment1 + '^||^' + comment2 + '^||^' + requestMessage;
			SaemteoActions.pushValues({type: "event", object: event});
			// 신청 처리
	...
	insertApplyForm = async () => {
		...
		try {
			...
			var params = {
				...
				eventAgreeCount: eventAnswer.checkEvent1 + eventAnswer.checkEvent2,	//이벤트 참여 개수 확인
			};
	...
	render() {
		...
		const {event, eventAnswer} = this.props;
		const {phoneCheckMessage, phoneCheckClassName, requestAmount, requestMessage, requestMessageLength} = this.state;
			...
			<section className="vivasamter">
				...
				<div className="applyDtl_top">
					<div className="applyDtl_cell ta_c pick">
						<h3><strong><span >비상한 선생님 + 비상한 우리반 </span> 신청하기</strong></h3>
					</div>
				</div>
					...
					<div className="vivasamter_applyDtl pdside0">
						<div className="pdside20 pb25">
							...
							<div className="combo_box type1 mb25">
								...
								<div className={"input_wrap receiveEtc " + (event.receive === '기타' ?  '' : 'hide')}>
									<input
										type="text"
										autoCapitalize="none"
										name="receiveEtc"
							...
							<div className={(eventAnswer.checkEvent2) === 1 ? '' : 'hide'}>
								<h2 className="info_tit mt30">
									<label htmlFor="ipt_phone">학급 인원 수</label>
								</h2>
								<div className='input_wrap mt5 receiveGradeClass'>
									<input
										type="number"
								...
								<h2 className="info_tit mt30">
									<label htmlFor="ipt_phone">스트랩 메시지</label>
								</h2>
								<div className="input_wrap">
									<input
										type="text"
										...
										/>
									<div className="count">
										<span>{requestMessageLength}/20</span>
									</div>
								</div>
							</div>
						</div>
						<div className="acco_notice_list pdside20">
							<div className="acco_notice_cont">
								...
								<ul className="privacyList event2">
									...
									<li>수집하는 개인정보: 성명, 재직 학교, 학교 소재지, 휴대전화번호</li>
									<li>개인정보 보유 및 이용기간: 2022년 4월 30일까지<br />(이용목적 달성 시 즉시 파기)</li>
									<li>연락처 오류 시 경품 재발송이 불가능합니다.<br /> 개인정보를 꼭 확인해 주세요.</li>
									<li>선물 발송을 위해 개인정보(성명, 재직 학교, 학교 소재지, 휴대전화번호)가 배송업체에 제공됩니다.<br/>((주)다우기술 사업자등록번호 220-81-02810),<br/>((주)한진 사업자등록번호 201-81-02823)</li>
								</ul>
								<br />
						...
						<button
							...
							className="btn_event_apply btn_full_on  mt35">신청완료
						</button>
					</div>
				</div>
			</section>
