...
class EventApply extends Component {
    ...
    componentDidMount(){
        ...
        if((eventAnswer.eventAnswerContent.Q1 == null) || (typeof eventAnswer.eventAnswerContent.Q1 == "undefined")
            ...
            common.error("선택하신 자료집의 정보가 없습니다. 다시 선택해주세요.");
    ...
    applyButtonClick = (target) => {
        ...
        try {
            ...
            if(eventAnswer.eventAnswerContent.Q1 === ""){
                common.info("해당 자료집 선택이 확인되지 않습니다. 다시 확인 부탁드립니다.");
                history.push('/saemteo/event/view/'+eventId);
            }
            if(event.userInfo === 'N'){
                event.eventAnswerDesc = event.inputType + '/' +event.schName + '/' +event.cellphone + '/' +event.schZipCd + '/' +event.schAddr + '/' +event.addressDetail + '/수령처 : ' + receive + '/ 선택한 자료집 : ' + eventAnswer.eventAnswerContent.Q1;
            }else{
                event.eventAnswerDesc = event.inputType + '/' +event.schName + '/' +event.cellphone + '/' +event.schZipCd + '/' +event.schAddr  + '/수령처 : ' + receive + '/ 선택한 자료집 : ' + eventAnswer.eventAnswerContent.Q1;
            }
            SaemteoActions.pushValues({type:"event", object:event});
    ...
    checkEventAmout = async () => {
        ...
        // 한번더 수량 체크
        if(eventAnswer.eventAnswerContent.eventCheck1 == true){
            try {
                params.eventAnswerDesc = "공모전수상작";
                params.eventType = "1"; // 수량 제한시 Type 변경
                const response = await api.eventAnswerDescCheck({...params});
                const response2 = await api.eventCheckLimitAmount({...params});
                if (response.data.amount >= response2.data.eventTotCnt) {
                    common.error("해당 자료집이 모두 소진되어 신청이 마감되었습니다.");
                    history.push('/saemteo/event/view/'+eventId);
                    return;
                }
            } catch (e) {}
        }
        if(eventAnswer.eventAnswerContent.eventCheck2 == true){
            try {
                params.eventAnswerDesc = "34음악수업";
                params.eventType = "2";  // 수량 제한시 Type 변경
                const response = await api.eventAnswerDescCheck({...params});
                const response2 = await api.eventCheckLimitAmount({...params});
                if(response.data.amount >= response2.data.eventTotCnt){
                    common.error("해당 자료집이 모두 소진되어 신청이 마감되었습니다.");
        ...
        if(eventAnswer.eventAnswerContent.eventCheck3 == true){
            try {
                params.eventAnswerDesc = "56음악수업";
                params.eventType = "3";  // 수량 제한시 Type 변경
                const response = await api.eventAnswerDescCheck({...params});
                const response2 = await api.eventCheckLimitAmount({...params});
                if(response.data.amount >= response2.data.eventTotCnt){
                    common.error("해당 자료집이 모두 소진되어 신청이 마감되었습니다.");
        ...
        if(eventAnswer.eventAnswerContent.eventCheck4 == true){
            try {
                params.eventAnswerDesc = "실과수업";
                params.eventType = "4";  // 수량 제한시 Type 변경
                const response = await api.eventAnswerDescCheck({...params});
                const response2 = await api.eventCheckLimitAmount({...params});
                if(response.data.amount >= response2.data.eventTotCnt){
                    common.error("해당 자료집이 모두 소진되어 신청이 마감되었습니다.");
    ...
    insertApplyForm = async () => {
        ...
        try {
            ...
            if(response.data.code === '1'){
                ...
                event.eventAnswerDesc = eventAnswer.eventAnswerContent.Q1; // 질문 응답 ( 해당되는 JSON의 Key를 이용해 Value를 가져온다. )
                        ...
                        <div className="acco_notice_list">
                            ...
                            <div className="acco_notice_cont mt10">
                                <ul class="policy">
	                                ...
	                                <li>- 개인정보 보유 및 이용기간 : 2020년 5월 31일까지 (이용목적 달성 시 즉시 파기)</li>
	                                <li>- 수집하는 개인정보의 취급위탁 : 자료집 발송을 위해 개인정보(이름/주소/연락처)를 배송업체에 취급 위탁 <br />(㈜한진 – 사업자등록번호 : 201-81-02823)</li>
                                </ul>
