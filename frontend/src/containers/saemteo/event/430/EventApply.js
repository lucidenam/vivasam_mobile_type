	...
	insertApplyForm = async () => {
		...
		try {
			...
			if (response.data.code === '1') {
				...
				const endDate = new Date('2023-2-16'); //실서버 일자로 변경되었는지 체크 요망
				const nowDate = new Date();

				if(endDate == nowDate) {
					common.error("준비한 선물이 모두 소진되어 이벤트가 종료되었습니다.");
				} else {
			...
			<section className="vivasamter">
				...
				<div className="applyDtl_top top_yell">
					<div className="applyDtl_cell ta_c pick">
						<h3><strong>2023 벽걸이 달력 배포 이벤트</strong> 신청하기</h3>
					</div>
				</div>
						...
						<div className="acco_notice_list pdside20">
							<div className="acco_notice_cont">
								...
								<ul className="privacyList">
									...
									<li>개인정보 보유 및 이용기간 : 2023년 3월 31일까지 <br/>(이용목적 달성 시 즉시 파기)</li>
									<li>개인정보 오기재 시 경품 재발송이 불가능합니다.<br/> 개인정보를 꼭 확인해 주세요.</li>
									<li>선물 발송을 위해 개인정보(성명, 학교주소, 휴대전화번호)가<br/> 배송업체에 제공됩니다.</li>
