...
class EventApply extends Component {
    ...
    applyButtonClick = (target) => {
        ...
        try {
            ...
            if(eventAnswer.eventAnswerContent.Q2 == ""){
                common.info("선택(입력)정보가 없습니다. 다시 선택(입력) 해주세요.");
                history.push('/saemteo/event/view/'+eventId);
                return false;
            }

            let eventCheckAnswer = eventAnswer.eventAnswerContent.Q1;
            ...
            event.eventAnswerDesc = event.inputType + '/' +event.schName + '/' +event.cellphone + '/' +event.schZipCd + '/' +event.schAddr + ' ' +event.addressDetail + '/수령처 : '+receive +'/콘텐츠 : '+eventCheckAnswer;
