...
class EventApply extends Component {
	...
	validateInfo = () => {
		...
		if (!event.userName) {
			...
			obj.message = '휴대전화번호를 입력해주세요.';
		} else if (eventAnswer.eventId == 504) {
			if (!event.agree) {
				obj.message = '모든 필수 동의 선택 후 이벤트 신청을 완료해주세요.';
			}else{
				obj.result = true;
			}
		} else if(eventAnswer.eventId == 505) {
			if (!event.agree1) {
				obj.message = '모든 필수 동의 선택 후 이벤트 신청을 완료해주세요.';
			} else {
				obj.result = true;
	...
	applyButtonClick = (target) => {
		...
		if (eventAnswer.eventId == 504) {
			receiveInfo = userInfo + '/' + event.schName + '/' + event.cellphone;
		}

		if (eventAnswer.eventId == 505) {
			receiveInfo =  event.cellphone;
		}
			...
			<section className="vivasamter">
				{
					eventAnswer.eventId === 504
						...
						<Fragment>
							...
							<div className="applyDtl_top">
								<div className="applyDtl_cell ta_c pick">
									<h3><strong>모여봐요~ 비바샘! 이벤트 – 기존 회원 선생님</strong></h3>
								</div>
							</div>
									...
									<div className="acco_notice_list pdside20 notice_sec mt0">
										<div className="acco_notice_cont">
											...
											<ul className="privacyList event2">
												...
												<li>개인정보 보유 및 이용 기간: 2024년 7월 31일까지 <br/>(이용목적 달성 시 즉시 파기)​</li>
												<li>개인정보 오기재로 인한 경품 재발송은 불가능합니다. <br/>개인정보를 꼭 확인해 주세요.​</li>
												<li>경품 발송을 위한 개인정보(성함, 휴대 전화번호)가 기프티콘 발송 서비스사에 제공됩니다.​ <br/>(㈜카카오 사업자등록번호 : 120-81-47521)​ <br/>(㈜다우기술 사업자등록번호: 220-81-02810)​ <br/>(㈜모바일이앤엠애드 사업자등록번호:215-87-19169)​</li>
											</ul>
						...
						<Fragment>
							...
							<div className="applyDtl_top">
								<div className="applyDtl_cell ta_c pick">
									<h3><strong>모여봐요~ 비바샘! 이벤트 – 신규 회원 선생님</strong></h3>
								</div>
							</div>
							<div className="vivasamter_apply">
								<div className="vivasamter_applyDtl pdside0">
