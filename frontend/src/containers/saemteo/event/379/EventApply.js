...
class EventApply extends Component {
	...
	componentDidMount() {
		...
		else if (!eventAnswer.eventAnswerDesc1 && !eventAnswer.eventAnswerDesc2) {
	...
	validateInfo = () => {
		...
		if (!event.userName) {
			...
			obj.message = '휴대전화번호를 입력해주세요.';
		} else if (eventAnswer.eventAnswerDesc2 != "" && event.amount === "") {
			obj.message = '신청 수량을 입력하세요.';
		} else if (!event.agree) {
	...
	applyButtonClick = (target) => {
		...
		let comment1 = eventAnswer.eventAnswerDesc1;
		if(comment1 == '') {
			comment1 = '미참여';
		}
		let comment2 = eventAnswer.eventAnswerDesc2;
		let amount = event.amount;
		if(comment2 == '') {
			comment2 = '미참여';
		} else {
			receiveInfo += '/이벤트2 신청수량 : ' + amount;
		}
	...
	render() {
		...
		return (
			<section className="vivasamter event379">
				<h2 className="blind">
					비바샘터 신청하기
				</h2>
				<div className="applyDtl_top">
					<div className="applyDtl_cell ta_c pick">
						<h3>아이들과 마음을 나누는 100가지 방법 <strong>-'마음'교과서</strong></h3>
					</div>
				</div>
				<div className="vivasamter_apply">
					<div className="vivasamter_applyDtl pdside0">
						<div className="pdside20 pb25">
							...
							{
								eventAnswer.eventAnswerDesc2 != '' &&
								<h2 className="info_tit">
									<span className="applyTxt"></span>
									<label htmlFor="ipt_phone" className="ipt_book">‘마음’ 신청 수량</label>
								</h2>
							}
							{
								eventAnswer.eventAnswerDesc2 != '' &&
								<div className="input_wrap">
									<input
										...
										name="amount"
										value={event.amount}
										className="input_sm mb10 input_book"
										onChange={this.handleChange}
										maxLength="2"/>
									<span className="input_book_txt">권</span>
									<span className="input_book_info">* 30권까지 신청 가능합니다.</span>
								</div>
							}
						</div>
						<div className="acco_notice_list pdside20">
							<div className="acco_notice_cont">
								...
								<ul className="privacyList">
									...
									<li>수집하는 개인정보 : 성명, 학교명, 학교주소, 휴대전화번호</li>
									<li>개인정보 보유 및 이용기간: 2021년 11월 30일까지<br />(이용목적 달성 시 즉시 파기)</li>
									<li>연락처 오류 시 경품 재발송이 불가능합니다. 개인정보를 꼭 확인해 주세요.</li>
									<li>선물 발송을 위해 개인정보(성명, 학교주소, 휴대전화번호)가 배송업체에 제공됩니다.<br/>
										((주)다우기술 사업자등록번호 220-81-02810),<br/>CJ대한통운(주) 사업자등록번호 : 110-81-05034)</li>
								</ul>
								<br />
