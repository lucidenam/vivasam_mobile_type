...
class EventApply extends Component {
    ...
    componentDidMount(){
        ...
        if((eventId == null) ||  (typeof eventId == "undefined") ||
            (eventAnswer.eventAnswerContent.selContents == null) ||  (typeof eventAnswer.eventAnswerContent.selContents == "undefined")
        ){
    ...
    applyButtonClick = (target) => {
        ...
        try {
            if(eventAnswer.eventAnswerContent.selContents.length){
                common.error("선택(입력)정보가 없습니다. 다시 선택(입력) 해주세요.");
                history.push('/saemteo/event/view/'+eventId);
                return false;
            }

            let strContents = eventAnswer.eventAnswerContent.selContents[0] + ' , ' + eventAnswer.eventAnswerContent.selContents[1];

            event.eventAnswerDesc = event.inputType + '/' +event.schName + '/' +event.cellphone + '/' +event.schZipCd + '/' +event.schAddr + ' ' +event.addressDetail + '/수령처 : ' + receive + '/장점 : '+strContents;
            SaemteoActions.pushValues({type:"event", object:event});
            // 신청 처리
    ...
    insertApplyForm = async () => {
        ...
        try {
            ...
            if(response.data.code === '1'){
                ...
                event.eventAnswerDesc = eventAnswer.eventAnswerContent.selContents[0] + ' , ' + eventAnswer.eventAnswerContent.selContents[1]; // 사연
            ...
            <section className="vivasamter">
                ...
                <div className="applyDtl_top">
                    <div className="applyDtl_cell pick">
                        <h3><strong>비상교육 고등 기본 수학</strong> 이벤트 참여하기</h3>
                    </div>
                </div>
                <div className="vivasamter_apply">
                    <div className="vivasamter_applyDtl">
                        ...
                        <p className="mt10 mb25 c_gray">
                            * 정확하지 않은 정보로 반송된 경우 재발송이 불가능합니다.<br />
                        </p>
                        <div className="acco_notice_list">
                            <div className="acco_notice_cont mt10">
                                ...
                                <ul class="policy">
                                    ...
                                    <li>- 수집하는 개인정보의 처리 위탁 : 경품 발송을 위한 개인정보<br />(성명/학교 소재지/연락처)를 배송업체에 처리 위탁<br />(㈜한진 사업자등록번호:201-81-02823)</li>
                                </ul>
                                <p className="comt mt10">* 선생님께서는 개인정보의 수집 및 이용, 처리 위탁에 대한 동의를 거부할 수 있습니다. 단, 동의를 거부할 경우 신청이 불가합니다.</p>
                            </div>
                        ...
                        <button
                            ...
                            className="btn_full_on2 mt20">등록</button>
                    </div>
                </div>
            </section>
