...
class EventApply extends Component {
	...
	openPopupSchool = (e) => {
		...
		e.preventDefault();
		PopupActions.openPopup({title:"학교 검색", component:<EventFindSchool handleSetSchool={this.handleSetSchool}/>});
	}
	...
	applyButtonClick = (target) => {
		...
		if(event.isCompanion === "Y") {
			if(!event.noCompanionInfo) {
				companionInfo += " 동반 선생님 있음" + "^||^" + event.companionName + '/' + event.companionSchool;
			} else {
	...
	insertApplyForm = async () => {
		...
		try {
			...
			if (response.data.code === '1') {
				...
				// history.replace('/saemteo/event/view/'+eventId);
			...
			<section className="vivasamter">
				...
				<div className="applyDtl_top">
					<div className="applyDtl_cell ta_c pick">
						<h3><strong>49차 교사문화프로그램 신청하기</strong></h3>
					</div>
					<div className="applyDtl_cell ta_c pick">
						<p>
				...
				<div className="vivasamter_apply">
					<div className="vivasamter_applyDtl pdside0">
						<div className="pdside20 pb25">
							...
							<p className="evtForm_labelTxt">*동반 선생님의 경우 최대 1명까지만 가능합니다.</p>
							...
							<div className="input_wrap info_wrap">
								...
								<p className="evtForm_labelTxt mt10">*<span className="c_o">선생님을 대상으로 하는 프로그램으로 선생님만 입장이 가능</span>합니다</p>
								<p className="evtForm_labelTxt">*동반 선생님이 아직 정해지지 않은  경우에는<br /> 미정에 체크해주세요.</p>
							</div>
							<h2 className="info_tit tit_flex textarea_tit">
								<label htmlFor="ipt_phone">신청 사연 <span className="c_o">(200자 이내)</span></label>
						...
						<div className="acco_notice_list pdside20">
							<div className="acco_notice_cont">
								...
								<ul className="privacyList event1">
									...
									<li>당첨이 확정된 경우 당일 현장에서 본인 확인 후 입장 가능하며,<br/>타인에게 양도는 불가합니다.</li>
									...
									<li className="c_o">프로그램이 끝나고 정성스러운 후기를 남겨주신 3분을 선정하여 소정의 선물을 드립니다.<br/>(해당 이벤트는 행사 당일 자세하게 안내해드리겠습니다)</li>
						...
						<div className="acco_notice_list pdside20 notice_sec mt0">
							<div className="acco_notice_cont">
								...
								<ul className="privacyList event2">
									...
									<li>개인정보 보유 및 이용기간 : 2024년 4월 30일까지(이용목적 달성 시 즉시 파기)</li>
								</ul>
								<br />
								<p className="privacyTxt">선생님께서는 개인정보의 수집 및 이용, 취급 위탁에 대한 동의를 거부할 수 있습니다. 단, 동의를 거부할 경우 교사문화 프로그램 신청이 불가합니다.</p>
							</div>
						</div>
						<div className="checkbox_circle_box pdside20 acco_notice_list notice_sec">
							<div className="acco_notice_cont">
								...
								<label
									...
									className="checkbox_circle_simple">
									<strong className="checkbox_circle_tit">
										<span className="c_o">프로그램 일정 및 장소(2024년 2월 24일 토요일,<br/>혜화역 극장 업스테이지)</span>를 확인하였습니다.
									</strong>
								</label>
							</div>
						</div>
