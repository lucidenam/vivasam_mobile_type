...
class EventApply extends Component {
	...
	validateInfo = () => {
		...
		if (!event.userName) {
			...
			obj.message = '휴대전화번호를 입력해주세요.';
		} else if (eventAnswer.evtTabId === 1 && event.schName.includes("고등")) {
			obj.message = "중학교 교사만 참여 가능한 이벤트입니다. \n참여를 원하시면 회원정보를 수정해주세요.";
		} else if ( eventAnswer.evtTabId === 2 && event.schName.includes("중학")) {
			obj.message = "고등학교 교사만 참여 가능한 이벤트입니다. \n참여를 원하시면 회원정보를 수정해주세요.";
		} else if (!event.agree) {
			...
			<section className="vivasamter event240520">
				...
				<div className="applyDtl_top">
					<div className="applyDtl_cell ta_c pick">
						<h3><strong>중고등 진로·진학 자료집 배포 이벤트</strong> 신청하기</h3>
					</div>
				</div>
						...
						<div className="acco_notice_list pdside20">
							<div className="acco_notice_cont">
								...
								<ul className="privacyList">
									...
									<li>개인정보 보유 및 이용 기간: 2024년 7월 30일까지(이용 목적 달성 시 즉시 파기)</li>
									<li>개인정보 오기로 인한 경품 재발송은 불가능합니다. 개인정보를 꼭 확인해 주세요.</li>
									<li>경품 발송을 위한 개인정보(성명/주소/휴대 전화 번호)가 배송 업체 및 비상교육 지사에 제공됩니다. <br/>(주)CJ대한통운 사업자번호: 110-81-05034 <br/>(주)한진택배 사업자등록번호: 201-81-02823</li>
								</ul>
								<br />
