...
class EventApply extends Component {
	...
	validateInfo = () => {
		...
		if (!event.schName) {
			...
			// }
		else if (eventAnswer.eventId == 466 && !event.agree1) {
			obj.message = '유의 사항 및 개인정보 수집 및 이용 사항에 동의해 주세요.';
		} else if (eventAnswer.eventId == 467 && !event.agree2) {
			obj.message = '유의 사항 및 개인정보 수집 및 이용 사항에 동의해 주세요.';
		} else {
			obj.result = true;
		}
	...
	applyButtonClick = (target) => {
		...
		let answer =  eventAnswer.eventId === 466 ? event.eleCont : event.highCont;

		if(answer === '비바샘 음악관') {
			answer = '^||^' + answer;
		} else if(answer === '음악샘 비버관') {
			answer = '^||^^||^' + answer;
		} else if(answer === '비바샘 흥얼관') {
			answer = '^||^^||^^||^' + answer;
		} else if(answer === '과학 가상 실험실') {
			answer = '^||^' + answer;
		} else if(answer === '비버샘 비밀 실험실') {
			answer = '^||^^||^' + answer;
		} else if(answer === '과상 과학 실험실') {
			answer = '^||^^||^^||^' + answer;
		}
					...
					eventAnswer.eventId === 466
						...
						<Fragment>
							...
							<div className="applyDtl_top ">
								<div className="applyDtl_cell ta_c pick">
									<h3>
										Don't worry 퀴즈쇼 이벤트 신청하기<br />
										<strong className="ele">(초등 선생님 대상)</strong>
									</h3>
								</div>
							</div>
									...
									<div className="acco_notice_list pdside20 mt0">
										<div className="acco_notice_cont">
											...
											<ul className="privacyList type02 ">
												<li>개인정보 수집 및 이용 동의 이용 목적 : 경품 발송 및 고객문의 응대​​</li>
												<li>
													수집하는 개인정보 : 성명, 재직학교, 주소, 휴대전화번호, 유튜브 계정
												</li>
												<li>
													에어팟 경품의 경우, 제세공과금(22%)이 부여되며
													제세공과금 처리를 위한 개인정보 수급 (신분증 사본, 주민등록번호, 계좌번호)을 위해 별도로 안내를 드릴 예정입니다.
												</li>
												<li>
													개인정보 보유 및 이용기간 : 2023년 10월 30일까지<br />
													(이용목적 달성 시 즉시 파기)
												</li>
												<li>
													이벤트 당첨자의 잘못된 개인정보 전달로 인한 불이익<br />
													(연락 불가, 경품 반송/분실 등)은 책임지지 않습니다
												</li>
												<li>
													선물 발송을 위해 개인정보(성명, 휴대전화번호)가 서비스사와 배송업체에 제공됩니다.<br />
													(㈜카카오 사업자등록번호 120-81-47521<br />
													(㈜한진 사업자등록번호: 201-81-02823)<br />
						...
						<Fragment>
							...
							<div className="applyDtl_top ">
								<h3>
									Don't worry 퀴즈쇼 이벤트 신청하기<br />
									<strong className="high">(중고등 선생님 대상)</strong>
								</h3>
							</div>
							<div className="vivasamter_apply ">
								<div className="vivasamter_applyDtl type02 pdside0">
