...
class EventApply extends Component {
	...
	validateInfo = () => {
		...
		if (!event.schName) {
			...
			obj.message = '휴대전화번호를 입력해 주세요.';
		} else if (eventAnswer.eventId == 471 && event.receive === "") {
			obj.message = '수령처를 선택해주세요.';
		} else if (eventAnswer.eventId == 471 && event.receive === "교실" && (event.receiveGrade === "" || event.receiveClass === "")) {
			obj.message = '학년 반을 입력해주세요.';
		} else if (eventAnswer.eventId == 471 && event.receive === "기타" && event.receiveEtc === "") {
			obj.message = '수령처를 입력해주세요.';
		} else if (eventAnswer.eventId == 471 && !event.agree1) {
			obj.message = '필수 동의 선택 후 이벤트 신청을 완료해 주세요.';
		} else if (eventAnswer.eventId == 472 && !event.agree2) {
			obj.message = '필수 동의 선택 후 이벤트 신청을 완료해 주세요.';
		} else {
			obj.result = true;
		}
						...
						<Fragment>
							...
							<div className="applyDtl_top ">
								<div className="applyDtl_cell ta_c pick">
									<h3>
										<strong>비바샘 10주년 이벤트</strong> 신청하기
									</h3>
								</div>
							</div>
									...
									<div className="acco_notice_list pdside20 mt0">
										<div className="acco_notice_cont">
											...
											<ul className="privacyList type02 ">
												...
												<li>수집하는 개인 정보: 성명, 재직 학교, 주소, 휴대전화번호</li>
												<li>
													개인정보 보유 및 이용 기간: 2023년 12월 31일까지(이용목적 달성 시 즉시 파기)
												</li>
												<li>
													개인정보 오기로 인한 경품 재발송은 불가능합니다. 개인정보를 꼭 확인해 주세요.
												</li>
												<li>
													경품 발송을 위한 개인정보(성함, 휴대 전화번호, 주소)가 배송업체 및 기프티콘 발송 서비스사에 제공됩니다.
												</li>
												<li>
													선물 발송을 위해 개인정보(성명, 휴대전화번호)가 서비스사와 배송업체에 제공됩니다.<br />
													(㈜CJ대한통운 사업자등록번호: 110-81-05034)/(㈜한진택배 사업자등록번호: 201-81-02823)<br />
													(㈜카카오 사업자등록번호 : 120-81-47521)/(㈜다우기술 사업자등록번호: 220-81-02810)/<br />
