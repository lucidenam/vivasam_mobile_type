...
class EventApply extends Component {
	...
	validateInfo = () => {
		...
		const {telephoneCheck, nowDate, day1, day2} = this.state;

		let reg_name = /[\uac00-\ud7a3]{2,4}/;
		let obj = {result: false, message: ''};
		let studentCnt = parseInt(event.studentCnt);

		const choosedBook = eventAnswer.eventAnswerContent;
		...
		if (!event.userName) {
			...
			obj.message = '휴대전화번호를 입력해주세요.';
		} else if (isNaN(studentCnt)) {
			obj.message = '학급 인원수를 입력해 주세요.';
		} else if (!(1 <= studentCnt && studentCnt <= 30)) {
			obj.message = '학급 인원수는 1명부터 최대 30명까지 입력 가능합니다.';
		} else if (nowDate === day1 && choosedBook === 'book1' && event.eachAmountLeft[0] < studentCnt) {
			obj.message = '선택하신 플래너의 잔여 수량보다 많은 수량을 입력하셨습니다.';
		} else if (nowDate === day1 && choosedBook === 'book2' && event.eachAmountLeft[1] < studentCnt) {
			obj.message = '선택하신 플래너의 잔여 수량보다 많은 수량을 입력하셨습니다.';
		} else if (nowDate === day2 && choosedBook === 'book1' && event.eachAmountLeft[2] < studentCnt) {
			obj.message = '선택하신 플래너의 잔여 수량보다 많은 수량을 입력하셨습니다.';
		} else if (nowDate === day2 && choosedBook === 'book2' && event.eachAmountLeft[3] < studentCnt) {
			obj.message = '선택하신 플래너의 잔여 수량보다 많은 수량을 입력하셨습니다.';
	...
	applyButtonClick = (target) => {
		...
		const {nowDate, nowHour, day1, day2} = this.state;
		...
		let answerNumber = eventAnswer.eventAnswerContent;
		console.log("answerNumber : "+answerNumber);

		let bookName = "";
		if(answerNumber === "book1") {
			bookName = "1번 플래너";
		} else {
			bookName = "2번 플래너";
		}
		...
		let studentCnt = "";
		if(parseInt(nowDate) === parseInt(day1) && answerNumber === "book1") {
			studentCnt = event.studentCnt + ",0,0,0"
		} else if(parseInt(nowDate) === parseInt(day1) && answerNumber === "book2") {
			studentCnt = "0," + event.studentCnt + ",0,0"
		} else if(parseInt(nowDate) === parseInt(day2) && answerNumber === "book1") {
			studentCnt = "0,0," + event.studentCnt + ",0"
		} else if(parseInt(nowDate) === parseInt(day2) && answerNumber === "book2") {
			studentCnt = "0,0,0," + event.studentCnt
		}
		...
		try {
			...
			event.eventAnswerDesc2 = bookName;
			event.ogStudentCnt = event.studentCnt;
			event.studentCnt = studentCnt;
			console.log(nowDate, day1, day2, studentCnt );
	...
	insertApplyForm = async () => {
		...
		const {nowDate, day1, day2} = this.state;
		...
		try {
			...
			var params = {
				...
				eventAnswerDesc2: event.eventAnswerDesc2 + "^||^" + event.ogStudentCnt + "명",
				cellphone: event.cellphone,
				userInfo: event.userInfo,
				schCode: event.schCode,

				amountYn: 'Y',   /* 수량제한 신청 */
				applyContentTotCnt: '4',     /* 상품 종류 수 */
				applyContentNumbers: '3,4,5,6',    /* 상품 seq, 복수신청시는 csv(3,4,5) */
				//applyTargetContentCnt: eventAnswer.answerNumber
				applyTargetContentCnt: event.studentCnt
			};
			console.log("eventAnswerDesc : "+params.eventAnswerDesc);
			console.log("eventAnswerDesc2 : "+params.eventAnswerDesc2);
			console.log("applyTargetContentCnt : "+params.applyTargetContentCnt);
			...
			if (response.data.code === '1') {
				...
				if(nowDate === day1 && (event.eachAmountLeft[0] < 1 || event.eachAmountLeft[1] < 1)) {
					common.error("오늘 준비한 플래너가 모두 소진되어 이벤트가 종료되었습니다. \n내일 오후 12시 정각에 다시 신청해주세요^^");
				} else if(nowDate === day2 && (event.eachAmountLeft[2] < 1 || event.eachAmountLeft[3] < 1)) {
					common.error("준비한 플래너가 모두 소진되어 신청이 마감되었습니다.");
				} else {
					common.error("선택하신 플래너 신청이 마감되었습니다.\n다른 플래너 선택 부탁드립니다!​");
				}
				history.push('/saemteo/event/view/'+eventId);
			} else {
	...
	render() {
		...
		return (
			<section className="vivasamter event220404">
				<h2 className="blind">
					비바샘터 신청하기
				</h2>
				<div className="applyDtl_top bg_yell">
					<div className="applyDtl_cell ta_c pick">
						<h3><strong>2023 드리밍 플래너 배포 이벤트</strong> 신청하기</h3>
					</div>
				</div>
				<div className="vivasamter_apply1">
					<div className="vivasamter_applyDtl pdside0">
						<div className="pdside20 pb25">
							...
							<h2 className="info_tit">
								<label htmlFor="ipt_address">재직 학교</label>
							</h2>
							...
							<p className="evtForm_labelTxt">* 최대 30명까지 입력 가능</p>
						</div>
						<div className="acco_notice_list pdside20">
							<div className="acco_notice_cont">
								...
								<ul className="privacyList">
									<li>개인정보 수집 및 이용동의이용 목적 : 경품 배송 및 고객문의 응대</li>
									<li>수집하는 개인정보 : 성명, 재직학교, 주소, 휴대전화번호</li>
									<li>개인정보 보유 및 이용기간 : 2023년 4월 30일까지 (이용목적
										달성 시 즉시 파기)
									</li>

									<li className="pt35 dot_none">선물 발송을 위해 개인정보(성명, 휴대전화번호)가 서비스사와 배송업체에 제공됩니다.
										(㈜CJ대한통운 사업자등록번호: 110-81-05034)/(㈜한진택배 사업자등록번호: 201-81-02823)
									</li>
								</ul>
								<br />
