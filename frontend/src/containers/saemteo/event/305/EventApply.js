...
class EventApply extends Component {
    ...
    componentDidMount(){
        ...
        if((eventId == null) || (typeof eventId == "undefined")){
            common.error("잘못된 접근 입니다.");
            history.push('/saemteo/event/view/'+eventId);
        }else if(this.state.eventAmount1 == 0 && this.state.eventAmount2 == 0){
            common.error("이벤트 신청이 마감되었습니다.");
            history.push('/saemteo/event/view/'+eventId);
        }else{
    ...
    handleCheckbook = (e) => {
        ...
        if(e.target.value == '1'){
            if(checked){
                this.setState({amount1: 1});
            }else{
                this.setState({amount1: 0});
            }
        }else if(e.target.value == '2') {
            if(checked){
                this.setState({amount2: 1});
            }else{
    ...
    validateInfo = () => {
        ...
        }else if(this.state.amount1 == 0 && this.state.amount2 == 0){
            obj.message = "신청자료를 선택해 주세요.";
        }else if(!event.agree){
    ...
    applyButtonClick = async(target) => {
        ...
        if(this.state.amount1 > 0) {
            try {
                ...
                if (chkQntCnt <= 0) { // 해당된 수량만큼 제한
                    common.error("중학교 역사 ① 수활북의 남은 수량이 없습니다.");
                    this.setState({eventAmount1: 0});
                    this.setState({amount1: 0});
                    return;
                }
        ...
        try {
            let checkBooks = "";
            if(this.state.amount1 == 1){
                checkBooks = "중학교 역사 ①";
            }
            if(this.state.amount2 == 1){
                if(checkBooks == ""){
                    checkBooks = "중학교 역사 ②";
                }else{
                    checkBooks = checkBooks+"^중학교 역사 ②";
                }
            }

            event.eventAnswerDesc = event.inputType + '/' +event.schName + '/' +event.cellphone + '/' +event.schZipCd + '/' +event.schAddr + ' ' +event.addressDetail + '/수령처 : ' + receive + '/' + checkBooks;
            SaemteoActions.pushValues({type:"event", object:event});
    ...
    insertApplyForm = async () => {
        const { eventId, event, history, SaemteoActions, PopupActions, BaseActions } = this.props;
        try {
            BaseActions.openLoading();
            event.eventAnswerSeq = 1;
            ...
            if(response.data.code === '1'){
                ...
                if(this.state.amount1 > 0) {
                    event.eventAnswerDesc = 1; // 수량
                    event.eventAnswerSeq = 3;
                    event.amountYn = "Y";
                    response = await SaemteoActions.setEventJoinAnswerAddInsert({...event});
                    if(response.data.code === '4'){
                        throw new Error("중학교 역사 ① 수활북의 남은 수량이 없습니다. 다시 신청해 주시기 바랍니다.");
                    }
                }
                if(this.state.amount2 > 0) {
                    event.eventAnswerDesc = 1; // 수량
            ...
            <section className="vivasamter">
                <h2 className="blind">
                    ...
                    <div className="applyDtl_cell">
                        <h3> 역사 선생님을 위한 <em>수활북 신청하기</em></h3>
                    </div>
                </div>
                    ...
                    <div className="vivasamter_applyDtl">
                        ...
                        <div className="input_wrap mb25">
                            <input
                                ...
                                onChange={this.handleChange}
                                value={event.schName}
                                className="input_sm school_name" />
                        </div>
                        ...
                        <div className="input_wrap">
                            <input
                                ...
                                className="input_sm school_address"
                                readOnly/>
                            <button
                                type="button"
                        ...
                        <h2 className="info_tit">
                            <label htmlFor="receive">수령처</label>
                        </h2>
                        ...
                        <h2 className="info_tit mt25">
                            <label htmlFor="ipt_cellphone">휴대전화번호</label>
                        </h2>
                        <div className="input_wrap mb25">
                            <input
                                type="tel"
                        ...
                        <h2 className="info_tit mt25">
                            <label htmlFor="ipt_cellphone">신청자료</label>
                        </h2>
                        <div className="input_wrap mb15">
                            <ul className="choice_book">
                                <li className="">
                                    <input
                                        id="choicebookY"
                                        type="checkbox"
                                        ...
                                        name="choicebook"
                                        value="1"
                                        checked={event.check1}
                                        onChange={this.handleCheckbook}
                                        disabled={!(this.state.eventAmount1 > 0)}
                                    />
                                    <label htmlFor="choicebookY">중학교 역사 ①</label>
                                </li>
                                <li className="">
                                    <input
                                        id="choicebookN"
                                        ...
                                        name="choicebook"
                                        value="2"
                                        checked={event.check2}
                                        onChange={this.handleCheckbook}
                                        disabled={!(this.state.eventAmount2 > 0)}
                                    />
                                    <label htmlFor="choicebookN">중학교 역사 ②</label>
                                </li>
                            </ul>
                        </div>
                        <div className="input_wrap mb40">
                            <ul className="txt">
                                <li>* 1종 1권씩, 총 2권 신청 가능합니다.</li>
                                <li>* 학교 번지수 및 수령처를 정확히 기입해 주세요.</li>
                                <li>* 7월 31일부터 순차적으로 발송됩니다.</li>
                            </ul>
                        </div>
                        ...
                        <div className="acco_notice_list">
                            <div className="acco_notice_cont">
                                ...
                                <ul className="policy">
                                    <li>&middot; 이용목적 : 수활북 발송
                                    </li>
                                    <li>&middot; 수집하는 개인정보 : 성명, 학교명, 학교 주소, 연락처
                                    </li>
                                    <li>&middot; 개인정보 보유 및 이용기간 : 2020년 8월 31일까지 (이용목적 달성 시 즉시 파기)
                                    </li>
                                    <li>&middot; 수집하는 개인정보의 취급위탁 : 자료집 발송을 위해 개인정보(이름/주소/연락처)를 배송업체에 취급 위탁 (㈜한진 사업자등록번호:201-81-02823)
                                    </li>
                                </ul>
                            ...
                            <div className="checkbox_circle_box mt10">
                                ...
                                <label
                                    ...
                                    <strong className="checkbox_circle_tit">본인은 개인정보 수집 및 이용내역을 확인하였으며, 이에 동의합니다.</strong>
                                </label>
                            </div>
                        </div>
                        <button
