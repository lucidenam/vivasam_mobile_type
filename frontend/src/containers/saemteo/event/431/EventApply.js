...
class EventApply extends Component {
	...
	validateInfo = () => {
		...
		} else if (event.career === "" || event.career < 1) {
	...
	applyButtonClick = (target) => {
		...
		try {
			...
			event.eventAnswerDesc += "/교사 경력 : " + event.career + "/비상교과서 채택 여부 : " + event.visangTbYN + '/' + event.cellphone;
			...
			<section className="vivasamter event220210">
				...
				<div className="applyDtl_top">
					<div className="applyDtl_cell ta_c pick">
						<h3><strong>비바샘 비바새미 10기 신청하기</strong></h3>
					</div>
				</div>
				<div className="vivasamter_apply">
					<div className="vivasamter_applyDtl pdside0">
									...
									<div className="selectbox select_sm mb25">
										...
										>
											<option value="">선택하세요</option>
											{
												...
												<option value="전과목(초등)" selected={event.mainSubject == "전과목(초등)"}>전과목(초등)</option>
											}
											...
											<option value="실과" selected={event.mainSubject == "실과"}>실과</option>
						...
						<div className="pdside20 pb25">
							<p className="pointTxt mt25">
								※ 각 학교급 교과와 비바샘 원격교육연수원을 중복 지원할 수 있습니다.​<br />
								(중복 지원을 하더라도 하나의 분야에 선발되어 활동하게 됩니다.)
							</p>
							<h2 className="info_tit">
								...
								<ul className="subject_field_list">
										...
										<ul>
											...
											<li>
												<input
													...
													name="applySubject"
													disabled={event.arrApplyContent1 === false}
													checked={event.arrApplyContent1 === true ? event.applySubject === '실과' : false}
													value="실과"
													onChange={this.handleChange}
												/>
												<label htmlFor="subjectE08">실과</label>
											</li>
									...
									<li>
										<input
											...
											disabled={this.state.TAB === 'E' && this.state.directInput === 'N'}
										/>
										<label htmlFor="subjectField02"><span>중 · 고등 교과</span> - 내 교과와 동일한 과목으로 자동 지원 됩니다.</label>
									</li>
									<li>
										<input
											id="subjectField03"
											type="checkbox"
											className="checkbox"
											name="arrApplyContent3"
											value="2"
											checked={event.arrApplyContent3}
											onChange={this.handleChange}
										/>
										<label htmlFor="subjectField03"><span>비바샘 원격교육연수원</span> - 교원 연수 제안 및 자문 활동</label>
									</li>
								</ul>
							</div>
							...
							<div className="acco_notice_cont">
								...
								<ul className="privacyList">
									...
									<li className="infoIndent">
										수집하는 개인정보 : 성명, 학교명, 담당 학년, 교과목, 교사 경력,<br />
										비상교과서 채택 여부, 휴대전화번호
									</li>
									<li>
										개인정보 보유 및 이용기간 : 2023년 3월 31일까지 (이용목적 달성<br />
										시 즉시 파기)</li>
									<li>
										수집하는 개인정보의 취급위탁 : 경품 발송을 위해<br />
										개인정보(이름/주소/연락처)를 서비스 업체에 취급 위탁<br />
										(CJ대한통운(주) 사업자등록번호 : 110-81-05034)
									</li>
								</ul>
								<br />
