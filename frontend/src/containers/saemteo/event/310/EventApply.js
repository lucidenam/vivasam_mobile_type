...
class EventApply extends Component {
    ...
    applyButtonClick = (target) => {
        ...
        try {
            ...
            let checkContents = eventAnswer.eventAnswerContent.Q1;
            let strContent = "";
            let strAmount = "";
            for(let i=3; i<=11; i++) {
                if (checkContents["check" + i]) {
                    if(strContent == ""){
                        strContent = this.state.contents["idx"+i];
                        strAmount = i+",1";
                    }else{
                        strContent = strContent + "," + this.state.contents["idx"+i];
                        strAmount = strAmount + "," + i + ",1";
                    }
                }
            }
            event.strAmount = strAmount;
            ...
            event.eventAnswerDesc = event.inputType + '/' +event.schName + '/' +event.cellphone + '/' +event.schZipCd + '/' +event.schAddr + ' ' +event.addressDetail + '/수령처 : '+receive +'/콘텐츠 : '+strContent;
    ...
    checkEventAmout = async () => {
        const { history, PopupActions, BaseActions , eventAnswer, eventId, SaemteoActions, event } = this.props;
        const params1 = {};
        ...
        if(amount){
            ...
            if(targetId != 0){
                common.error(this.state.contents["idx"+targetId]+" 활동집 신청이 마감되었습니다. 다시 입력해주세요.");
                history.push('/saemteo/event/view/'+eventId);
            }
        }
         **/
    ...
    insertApplyForm = async () => {
        ...
        try {
            ...
            let response = await SaemteoActions.insertEventApplyAll({...event});
            if(response.data.code === '1'){
                ...
                throw new Error(this.state.contents['idx'+response.data.seq]+" 활동지의 남은 수량이 없습니다. 다시 신청해 주시기 바랍니다.");
            }else{
                ...
                <div className="vivasamter_apply">
                    <div className="vivasamter_applyDtl">
                        ...
                        <div className="sub_text">
							<p>* 학교 주소 및 수령처를 정확히 기입해 주세요.</p>
							<p>* 잘못 기재된 주소 정보로 인해 반송된 선물은 재발송되지 않습니다.</p>
						</div>
                        ...
                        <div className="acco_notice_list">
                            ...
                            <div className="acco_notice_cont mt10">
                                <ul class="policy">
                                    ...
                                    <li>- 개인정보 보유 및 이용기간 : 2020년 10월 31일까지 (이용목적 달성 시 즉시 파기)</li>
                                    <li>- 수집하는 개인정보의 취급위탁 : 선물 발송을 위해 개인정보(이름/주소/연락처)를 배송업체에 취급 위탁<br/>
                                        (㈜한진 사업자등록번호:201-81-02823)</li>

                                </ul>
                                <p className="comt mt10">선생님께서는 개인정보의 수집 및 이용, 취급 위탁에 대한 동의를 거부할 수 있습니다. <br/>단, 동의를 거부할 경우 신청이 불가합니다.</p>
                            </div>
