...
class EventApply extends Component {
	...
	componentDidMount() {
		...
		console.log("apply test : " + eventAnswer.eventAnswerDesc);
	...
	applyButtonClick = (target) => {
		...
		try {
			...
			event.eventAnswerDesc2 = eventAnswer.eventAnswerDesc;
			SaemteoActions.pushValues({type: "event", object: event});
			// 신청 처리
	...
	insertApplyForm = async () => {
		...
		try {
			...
			var params = {
				...
				eventAnswerDesc2: event.eventAnswerDesc2 == "ES" ? "초등" : "중고등",
	...
	render() {
		...
		return (
			<section className="vivasamter event240503">
				<h2 className="blind">
					비바샘터 신청하기
				</h2>
				<div className="applyDtl_top">
					<div className="applyDtl_cell ta_c pick">
						<h3><strong>AI 디지털교과서 + 2022 개정 교육과정 2022% 대비하기</strong></h3>
					</div>
				</div>
