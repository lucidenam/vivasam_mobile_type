...
class EventApply extends Component {
	...
	validateInfo = () => {
		...
		if (!event.userName) {
			...
			obj.message = '휴대전화번호를 입력해주세요.';
		} else if (!this.state.eventContents) {
			obj.message = '기대평 & 후기를  작성해 주세요.'
		} else if (!event.agree) {
	...
	applyButtonClick = (target) => {
		...
		let answers = "";
		answers = this.state.eventContents;
		try {
			event.eventId = eventId;
			event.eventAnswerDesc = receiveInfo;
			event.eventAnswerDesc2 = answers;
			SaemteoActions.pushValues({type: "event", object: event});
			// 신청 처리
				...
				<Fragment>
						...
						<div className="applyDtl_top">
							<div className="applyDtl_cell ta_c pick">
								<h3><strong>뚝딱학습지 오픈 기념 리뷰 작성 이벤트</strong></h3>
							</div>
						</div>
							...
							<div className="vivasamter_applyDtl pdside0">
								<div className="pdside20 pb25">
									...
									<h2 className="info_tit tit_flex textarea_tit">
										<label htmlFor="ipt_phone">뚝딱학습지 리뷰</label>
									</h2>
									<div className="input_wrap">
										<textarea
												name="applyContent"
												...
												placeholder="<뚝딱학습지> 기대평&사용후기를 입력해 주세요. (100자 이내)​​"
												className="textarea">
										</textarea>
										<span className="count"><span className="currentCount">{this.state.eventLength}</span>/100</span>
									</div>
								</div>
								...
								<span className="privacyTit">
									개인정보 수집 및 이용동의
										...
										<ul className="privacyList event2">
											...
											<li>개인정보 보유 및 이용기간 : 2024년 6월 30일까지 ​​</li>
											<li>연락처 오류 시 경품 재발송이 불가능합니다. 개인정보를 꼭 확인해 주세요.​​</li>
											<li>선물 발송을 위해 개인정보(성명, 휴대전화번호)가 서비스사와 배송업체에 제공됩니다.​​ <br/>(㈜모바일이앤엠애드 사업자등록번호 215-87-19169)​</li>
										</ul>
