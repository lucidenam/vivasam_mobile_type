...
class EventApply extends Component {
    ...
    componentDidMount = async() => {
        const {BaseActions, eventId , eventAnswer , history, logged } = this.props;
    ...
    eventApplyCheck = async() => {
        const { SaemteoActions, eventId, history } = this.props;
        ...
        if(isEventOpen1){
            if(!chkAmountFull[0]){
                this.setState({chkContent1: true});
                this.setState({chkContent2: false});
            }
            // 실채점 선택 못함
            chkAmountFull[1] = true;
        }else if(isEventOpen2){
            if(!chkAmountFull[0]){
                this.setState({chkContent1: true});
                this.setState({chkContent2: false});
            }else if(chkAmountFull[0] && !chkAmountFull[1]){
                this.setState({chkContent1: false});
                this.setState({chkContent2: true});
            }else if(chkAmountFull[0] && chkAmountFull[1]){
                common.error("물량이 모두 소진되어 이벤트가 종료 되었습니다.");
                history.push('/saemteo/event/view/'+eventId);
            }
        }
    ...
    radioChange = (e) => {
        if(e.target.id === 'markingTbl01'){
            this.setState({chkContent1: e.target.checked});
            this.setState({chkContent2: !e.target.checked});
        }else if(e.target.id === 'markingTbl02'){
            this.setState({chkContent1: !e.target.checked});
            this.setState({chkContent2: e.target.checked});
        }
    }
    ...
    applyButtonClick = (target) => {
        ...
        try {
            // chkContent 확인
            let applyContent = '';
            if(this.state.chkContent1){
                applyContent = '배치표 : 3';
                event.applyTargetContentCnt = '1,0';
            }else if(this.state.chkContent2){
                applyContent = '배치표 : 4';
                event.applyTargetContentCnt = '0,1';
            }else{
                common.error("배치표를 선택해 주세요.");
                return false;
            }
            event.applyContent = applyContent;
            
            event.eventAnswerDesc = event.inputType + '/' +event.schName + '/' +event.cellphone + '/' +event.schZipCd + '/' +event.schAddr + ' ' +event.addressDetail + '/수령처 : ' + receive+'/'+applyContent;
            SaemteoActions.pushValues({type:"event", object:event});
            // 신청 처리
    ...
    insertApplyForm = async () => {
        ...
        try {
            ...
            event.amountYn = 'Y';
            event.eventAnswerDesc2 = event.applyContent;
            event.applyContentTotCnt = '2';
            event.applyContentNumbers = '3,4';
            //event.applyTargetContentCnt = '1,0';
            ...
            if(response.data.code === '1'){
                ...
                throw new Error("선택하신 배치표 신청 수량이 마감되었습니다.");
            }else{
        ...
        return (
            <section className="vivasamter">
                ...
                <div className="applyDtl_top">
                    <div className="applyDtl_cell pick ta_c">
                        <h3>2021학년도 <strong>정시 모집 배치표</strong> 신청하기</h3>
                    </div>
                </div>
                <div className="vivasamter_apply">
                    <div className="vivasamter_applyDtl">
                        ...
                        <h2 className="info_tit">
                            <label htmlFor="ipt_schname">재직 학교</label>
                        </h2>
                        ...
                        <h2 className="info_tit">
                            <label htmlFor="ipt_marking">배치표</label>
                        </h2>
                        <div className="input_wrap mb15">
                            <ul className="join_ipt_chk">
                                <li className="join_chk_list" style={{width:'45%'}}>
                                    <input
                                        ...
                                        name="markingTbl"
                                        value="Y"
                                        checked={this.state.chkContent1}
                                        onChange={this.radioChange}
                                        disabled={this.state.chkAmountFull[0]}
                                        />
                                    <label htmlFor="markingTbl01">가채점 배치표</label>
                                </li>
                                <li className="join_chk_list" style={{width:'45%'}}>
                                    <input
                                        id="markingTbl02"
                                        ...
                                        name="markingTbl"
                                        value="N"
                                        checked={this.state.chkContent2}
                                        onChange={this.radioChange}
                                        disabled={this.state.chkAmountFull[1]}
                                        />
                                    <label htmlFor="markingTbl02">실채점 배치표</label>
                                </li>
                            </ul>
                        </div>
                        <div className="acco_notice_list">
                            <div className="acco_notice_cont mt10">
                                ...
                                <ul class="policy">
	                                <li>- 이용목적 : 배치표 배송 및 CS응대</li>
                                    <li>- 수집하는 개인정보 : 성명, 학교 정보, 학교 주소, 연락처</li>
                                    <li>- 개인정보 보유 및 이용기간 : 2021년 1월 31일까지 <br/>(이용목적 달성 시 즉시 파기)</li>
                                    <li>- 수집하는 개인정보의 취급위탁 : 자료집 발송을 위해 개인정보(이름/주소/연락처)를<br />배송업체에 취급 위탁(㈜한진 사업자등록번호:201-81-02823)</li>
                                </ul>
                        ...
                        <button
                            ...
                            className="btn_full_on2 mt20">신청완료</button>
                    </div>
                </div>
            </section>
