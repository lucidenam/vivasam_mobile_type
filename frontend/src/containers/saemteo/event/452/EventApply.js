...
class EventApply extends Component {
	...
	validateInfo = () => {
		...
		} else if (event.schName.includes("초등") || event.schName.includes("중학") ) {
			obj.message = '고등 선생님만 참여하실 수 있는 이벤트입니다.';
		} else if (event.schZipCd === "" || event.schAddr === "") {
			obj.message = '학교주소를 입력해 주세요.';
		} else if (event.addressDetail === "") {
			obj.message = '학교 상세주소를 입력해 주세요.';
		} else if (event.addressDetail.includes("초등") || event.addressDetail.includes("중학") ) {
			obj.message = '고등 선생님만 참여하실 수 있는 이벤트입니다.';
	...
	render() {
		...
		return (
			<section className="vivasamter event230515">
				<h2 className="blind">
					비바샘터 신청하기
				</h2>
				<div className="applyDtl_top">
					<div className="applyDtl_cell ta_c pick">
						<h3><strong>2023년 고등 완자 기출 PICK 교사용 체험 이벤트</strong> 신청하기</h3>
					</div>
				</div>
						...
						<div className="acco_notice_list pdside20">
							<div className="acco_notice_cont">
								...
								<ul className="privacyList">
									...
									<li>수집하는 개인정보 : 성명,  재직학교, 수령처, 휴대전화번호</li>
									<li>개인정보 보유 및 이용기간 : 2023년 7월 31일까지<br/>(이용목적 달성 시 즉시 파기)</li>
									<li>연락처 오류 시 경품 재발송이 불가능합니다. 개인정보를 꼭 확인해 주세요.</li>
									<li>선물 발송을 위해 개인정보(성명, 수령처,  휴대전화번호)가 배송업체에 제공됩니다.<br/>((주)모바일이앤엠애드 사업자등록번호 215-87-19169),<br/>(주)CJ대한통운 사업자등록번호 : 110-81-05034))</li>
								</ul>
								<br />
								<p className="privacyTxt">선생님께서는 개인정보의 수집 및 이용, 취급 위탁에 대한 동의를 거부할 수 있습니다. 단, 동의를 거부할 경우 이벤트 참여가 불가합니다.</p>
							</div>
						</div>
