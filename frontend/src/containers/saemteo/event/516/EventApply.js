...
class EventApply extends Component {
	...
	validateInfo = () => {
		...
		} else if (eventAnswer.eventId == 518 && event.receive === "") {
			obj.message = '수령처를 선택해주세요.';
		} else if (eventAnswer.eventId == 518 && event.receive === "교실" && (event.receiveGrade === "" || event.receiveClass === "")) {
			obj.message = '학년 반을 입력해주세요.';
		} else if (eventAnswer.eventId == 518 && event.receive === "기타" && event.receiveEtc === "") {
			obj.message = '수령처를 입력해주세요.';
		} else if (eventAnswer.eventId == 518 && !event.receiveEtc) {
			obj.message = '학급 학생 수를 입력해주세요.';
		} else if (eventAnswer.eventId == 518 && this.state.eventContents === "") { // 내용 미입력
	...
	applyButtonClick = (target) => {
		...
		if (eventAnswer.eventId == 517) {
			receiveInfo = userInfo + '/' + event.schName + '/' + event.schZipCd + '/' + event.schAddr + '/' + event.addressDetail + '/' + event.cellphone;
		}
		if (eventAnswer.eventId == 518) {
			answerData =  receive + '/' + event.receiveEtc +'명' + '^||^' + this.state.eventContents;
		}
										...
										<div className="acco_notice_list pdside20">
											<div className="acco_notice_cont">
												...
												<ul className="privacyList">
													...
													<li>수집하는 개인정보: 성명, 재직 학교, 휴대전화번호, 학교 주소</li>
													<li>개인 정보 보유 및 이용 기간: 2024년 10월 30일까지(이용 목적 달성 시 즉시 파기)</li>
													<li>개인 정보 오기로 인한 경품 재발송은 불가능합니다. 개인 정보를 꼭 확인해 주세요.</li>
													<li>경품 발송을 위해 개인 정보가 서비스사와 배송 업체에 제공됩니다.<br/>(주)모바일이앤엠애드 사업자등록번호 : 215-87-19169 / 아기자기 선물가게
														사업자등록번호: 530-31-00427 / 우체국 택배 사업자등록번호: 206-83-02050
													</li>
												</ul>
											</div>
										</div>
									...
									<div className="applyDtl_inner pdside20 pb25">
										...
										<div className="input_wrap">
												<textarea
													...
													placeholder="투표 결과 및 2학기 학급 생활에 대한 기대평을 남겨주세요. (최대 200자)"
													className="ipt_textarea">
												</textarea>
										</div>
										<div className="count_wrap"><p className="count"><span>{this.state.eventLength}</span>/200</p>
										</div>
									</div>
									<div className="acco_notice_list pdside20">
										<div className="acco_notice_cont">
											...
											<ul className="privacyList">
												...
												<li>경품은 당첨자 발표 이후 순차적으로 발송되며, 학급 학생 대상 이벤트의 경우 간식 배송을 위해 선생님께 연락드릴 예정입니다.</li>
											</ul>
										</div>
									</div>
									<div className="checkbox_circle_box mt25 pdside20">
