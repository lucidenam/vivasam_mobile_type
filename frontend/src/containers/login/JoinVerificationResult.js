...
class JoinVerificationResult extends Component {
    ...
    setIdByCheckCase = async () => {
        ...
        this.getValidYnByMemberId(check.existMemberId);
    ...
    // memberId를 통해 info에 validYn insert
    getValidYnByMemberId = async (existMemberId) => {
    ...
    nextButtonClick = async (target) => {
        ...
        try {
            ...
            //비바샘 아이디가 있고 통합회원이 아닐 경우 바로 통합회원으로 전환
        ...
        if (!this.props.test) {
            if(check.sMessage != null && check.sMessage != '') {
                ...
                if (!check.isExistViva && !check.isExistTsch) {
                    ...
                    this.props.history.replace('/');
                    this.props.history.push('/join/info');
                }
            }
        } else {
            /**/
