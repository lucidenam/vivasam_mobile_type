...
class JoinInfo extends Component {
    ...
    componentDidMount() {
        ...
        let {agree, info, history, JoinActions, school, check, sso} = this.props;
    ...
    handleChange = (e) => {
        ...
        if (e.target.name !== "telephone")info[e.target.name] = e.target.value;
        JoinActions.pushValues({type: "info", object: info});
        ...
        if (e.target.name === "userId") {
            ...
            if(info.telephone == e.target.value) {
                this.setState({
                    telephoneCheck: true,
    ...
    duplicateIdClick = async (e) => {
        const {info, JoinActions} = this.props;
        const idCheck = /^[a-z0-9]{3,12}$/g;
        const pattern1 = /[0-9]/;
        const pattern2 = /[a-zA-Z]/;

        if(!idCheck.test(info.userId) || !pattern1.test(info.userId) || !pattern2.test(info.userId)) {
    ...
    duplicateEmailClick = async (e) => {
        ...
        // try {
        //     let {data} = await JoinActions.checkUserEmail('', info.email);
    ...
    snsSleepWakeUp = async (e) => {
        const { JoinActions, info } = this.props;
    ...
    validateInfo = () => {
        ...
        const {duplicateIdCheck, telephoneCheck, passwordRule, duplicateEmailCheck, file} = this.state;
        let obj = {result: false, message: '', focus: ''};

        if (!this.checkpassword()) {
            obj.message = '입력하신 비밀번호와 일치하지 않습니다.';
            obj.focus = this.refs.Checkpassword;
        } else if (info.type === 'IPIN' && info.telephone === "") {
    ...
    insertJoinAfterHome = async (target) => {
        if (this.state.findOnclick === false) {
            ...
            const {info, school, agree, sso} = this.props;
            const {duplicateIdCheck, telephoneCheck, passwordRule, duplicateEmailCheck, file, isRealizeId} = this.state;
            ...
            if (this.state.checkEvent450Progress && this.state.progressReco) {
                ...
                if (this.props.recoJoinInfo.reco) {
                    let response = await api.chkValidReco(this.props.recoJoinInfo.reco);
                    const validReco = response.data.valid;
            ...
            api.appConfirm('가입하시겠습니까?').then(confirm => {
                if (confirm === true) {
                    let code = this.insertJoinForm(target);
                } else {
                    this.setState({ // 가입 취소시 클릭 가능 상태 전환
                        findOnclick: false
                    });
    ...
    insertJoinForm = async (target) => {
        ...
        try {
            if (this.state.findOnclick === true) {
                ...
                let code = await JoinActions.insertJoin({ ...agree, ...check, ...info, ...school, ...recoJoinInfo, ...sso });
                if (code.data == 1) {
                    common.error("아이디를 입력해주세요.");
                    this.setState({
                        findOnclick: false
    ...
    render() {
        const {check, info, school, agree, sso, recommendInfo} = this.props;
        const {
            recommendMsg,
            idExist,
            passwordMessage,
                        ...
                        <div className="join_info">
                            ...
                            {(sso.newUserId !== undefined && sso.newUserId !== "") && (
                                <div className="input_wrap has_btn">
                                    <input type="text" id="ipt_id" name="userId" ref="userId" className="input_sm"
                                ...
                                <Fragment>
                                    ...
                                    <div className="input_wrap">
                                        <input
                                            ...
                                            maxLength="13"
                                            className="input_sm"
                                            readOnly={info.type === 'NICE' || telInputcelphoneReadOnly}/>
                                        {!telephoneCheck &&
                                        <button
                                            type="button"
                                            className="input_in_btn btn_gray"
                            ...
                            {(sso.newUserId === undefined || sso.newUserId === "") ?
                                <Fragment>
                                    ...
                                    <div className={"recommend_form"}
                                        ...
                                        <p className={"c_o recommendNoti pl17"}>*추천인 아이디는 교사인증을 완료한 선생님만 조회 가능합니다.</p>
                                    </div>
                                </Fragment>
                                :
                                ""
