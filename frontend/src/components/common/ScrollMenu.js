...
export class ScrollMenu extends React.Component {
  ...
  componentDidMount() {
    ...
    //custom
    setTimeout(() => this.setInitial(), 200);
  }
  ...
  setInitial = () => {
    try {
      const {selected, data, innerWrapperClass} = this.props;
      ...
      //custom
      if (innerWrapperClass === 'scrollpage') {
        var navWidth = data.length * 63;
        if (navWidth < window.innerWidth) {
          document.getElementById('viewerNav').style.width = navWidth + 'px';
        } else {
          document.getElementById('viewerNav').style.width = '';
        }
      } else if (innerWrapperClass === 'scrollmenu') {
        if (window.innerWidth < window.innerHeight) {
          if (parseInt(data.find(el => el.props.isActive === true).key) > 2) {
            document.getElementById(innerWrapperClass).style.transform = "translate3d(" + (window.innerWidth - 560) + "px,0,0)";
          } else {
            document.getElementById(innerWrapperClass).style.transform = "translate3d(-5px,0,0)";
          }
        } else {
          document.getElementById(innerWrapperClass).style.transform = "translate3d(" + ((window.innerWidth - 560) / 2) + "px,0,0)";
        }
      } else if (innerWrapperClass === 'liveScrollMenu') {
        if (window.innerWidth < window.innerHeight) { // 20190528 스크롤 추가, 해당 부분 스크롤 px을 URL에 맞게 지정
          if ("OnlineClassSurvivalSecret" == data.find(el => el.props.isActive === true).key) {
            document.getElementById(innerWrapperClass).style.transform = "translate3d(" + (window.innerWidth - 700) + "px,0,0)";
          } else if ("WhatTodayMaap" == data.find(el => el.props.isActive === true).key) {
            document.getElementById(innerWrapperClass).style.transform = "translate3d(" + (window.innerWidth - 1350) + "px,0,0)";
          } else if ("WhatToday3" == data.find(el => el.props.isActive === true).key) {
            document.getElementById(innerWrapperClass).style.transform = "translate3d(" + (window.innerWidth - 1200) + "px,0,0)";
          } else if ("WhatToday2" == data.find(el => el.props.isActive === true).key) {
            document.getElementById(innerWrapperClass).style.transform = "translate3d(" + (window.innerWidth - 1050) + "px,0,0)";
          } else if ("WhatToday" == data.find(el => el.props.isActive === true).key) {
            document.getElementById(innerWrapperClass).style.transform = "translate3d(" + (window.innerWidth - 900) + "px,0,0)";
          } else if ("classLiveQuestion" == data.find(el => el.props.isActive === true).key) {
            document.getElementById(innerWrapperClass).style.transform = "translate3d(" + (window.innerWidth - 750) + "px,0,0)";
          } else if ("elementaryProcessAppraisalList" == data.find(el => el.props.isActive === true).key) {
            document.getElementById(innerWrapperClass).style.transform = "translate3d(" + (window.innerWidth - 600) + "px,0,0)";
          } else {
            document.getElementById(innerWrapperClass).style.transform = "translate3d(-5px,0,0)";
          }
        } else {
          document.getElementById(innerWrapperClass).style.transform = "translate3d(" + ((window.innerWidth - 475) / 2) + "px,0,0)";
        }
      }
  ...
  getPagesOffsets = ({
    ...
    const firstPageOffset = window.innerWidth > window.innerHeight ? +this.getCenterOffset({ items: visibleItemsStart, menuWidth }).toFixed(3) : 0;
    ...
    const lastPageOffset = window.innerWidth > window.innerHeight ? +this.getCenterOffset({ items: visibleItemsEnd, menuWidth }).toFixed(3) : 0;
  ...
  onItemClick = id => {
    const { clickWhenDrag, onSelect, innerWrapperClass } = this.props;
  ...
  handleDragStart = ev => {
    ...
    const { dragging: draggingEnable,innerWrapperClass } = this.props;
    if (!draggingEnable) return false;
    let { translate: startDragTranslate } = this.state;
    if(innerWrapperClass === 'scrollpage'){
      var x = parseInt(document.getElementById("scrollpage").style.transform.replace(/translate3d|px|\(|\)/gi, '').split(',')[0]);
      if(startDragTranslate !== x){
        startDragTranslate = x;
      };
      this.setState({ dragging: true, xPoint: 0, translate: x, startDragTranslate, xDraggedDistance: 0 });
    }else{
